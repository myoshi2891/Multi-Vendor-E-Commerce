...
| 1-A-5 Green | `updateOrderItemStatusAsAdmin` + `updateOrderPaymentStatus`（DB のみ・決済 API 呼ばない）実装 + テスト。**UI には「DB のみ変更・決済 API 非連携」警告を表示**（[design §3.3](./design.md#33-ui)・C-a） | `feat(order): add admin item/payment status updates` |
...
- **回帰テスト必須観点**:
  ...
  - **STORE クーポンの「正しい割引額」は不変**。ただし `applyCoupon` の Number→`Prisma.Decimal` 化（上記 **5-B-3**）は**既存の浮動小数点丸めバグの修正を兼ねる**ため、従来の Number ベース出力とは**端数ケースで最大 1 セント差**が出うる。回帰テストは **Decimal 基準の正値**でアサートし、この差分は「退行」ではなく「バグ修正（挙動改善）」として扱う（リリースノート相当の挙動変化として記録）。`applyCoupon` が**唯一の権威ある計算面**であり、差分検証はここで行う。
