...
| 1-A-5 Green | `updateOrderItemStatusAsAdmin` + `updateOrderPaymentStatus`（DB のみ・決済 API 呼ばない）実装 + テスト | `feat(order): add admin item/payment status updates` |
...
- **要確認**: `enum StoreStatus`（[schema.prisma:73](../../../prisma/schema.prisma#L73)）の実値で `ACTIVE`/`PENDING` を合わせる。
...
| 5-B-6 Refactor | seller `upsertCoupon`（[coupon.ts:43-51](../../../src/queries/coupon.ts#L43-L51)）にも P2002 ハンドリング追加（他店舗/platform コード衝突対策） | `refactor(coupon): add P2002 handling to seller upsertCoupon` |
