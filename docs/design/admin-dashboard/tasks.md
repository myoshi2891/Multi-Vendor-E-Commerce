...
| 1-A-5 Green | `updateOrderItemStatusAsAdmin`（`OrderItem.status`=`ProductStatus` 更新・配送/履行系）+ `updateOrderPaymentStatus`（`Order.paymentStatus` を DB のみ更新・決済 API 呼ばない）実装 + テスト。**`updateOrderPaymentStatus` は `status === "Refunded" \| "Cancelled"` のとき、同一 `db.$transaction` 内で子 `OrderGroup`/`OrderItem` へ伝播する**（childStatus 写像: Refunded→`OrderStatus.Refunded` / Cancelled→`OrderStatus.Canceled`。外部決済 API は呼ばない・[design §3.1](./design.md#31-新規-querysrcqueriesorderts-に追加)/[§3.2](./design.md#32-親子ステータス連動判断6-2-のステートマシン)・AC-F2-5）。**「DB のみ変更・決済 API 非連携」警告は `updateOrderPaymentStatus` の操作 UI にのみ表示**（`updateOrderItemStatusAsAdmin` には付与せず、必要なら配送キャリア未連携の手動変更である旨のみ示す）（[design §3.3](./design.md#33-ui)・[§3.5 runbook](./design.md#35-決済ステータス手動変更運用手順runbook)・C-a） | `feat(order): add admin item/payment status updates` |
| 1-A-6 Refactor | 監査ログ（`[Admin:Action] actor=... target=... to=...`）を各 action に付与 | `refactor(order): add structured audit logs to admin actions` |

- **テスト必須観点**: 非 ADMIN 拒否（a/b/c 3 階層）、limit=500000 → 100 にキャップ（AC-F2-3）。`updateOrderPaymentStatus`/`updateOrderItemStatusAsAdmin` の親子連動として: (a) 親 `Order` を `Cancelled`/`Refunded` に変更すると子 `OrderGroup`/`OrderItem` が**同一 `db.$transaction` でアトミック更新**される（AC-F2-5）、(b) 決済 API（Stripe/PayPal）が**呼ばれない**（AC-F2-6・モック検証）、(c) 親 Canceled → 子 Canceled の写像（[design §3.2](./design.md#32-親子ステータス連動判断6-2-のステートマシン)）。
...
| 5-B-6a Red | seller `upsertCoupon`（[coupon.ts:43-51](../../../src/queries/coupon.ts#L43-L51)）の **P2002 フォールバック回帰テスト**: `findFirst` をすり抜ける並行挿入をシミュレートし（`db.coupon.upsert` モックが `code @unique` 違反の `Prisma.PrismaClientKnownRequestError`（`code: "P2002"`）を throw）、ハンドラが統一日本語メッセージ「このクーポンコードは既に使用されています」を返す/throw することを **失敗確認**。テスト名・アサートに `upsertCoupon` / `coupon.ts` / `P2002` を含める（AC-F3-2） | `test(coupon): add failing P2002 fallback test for seller upsertCoupon` |
| 5-B-6 Refactor | seller `upsertCoupon`（[coupon.ts:43-51](../../../src/queries/coupon.ts#L43-L51)）にも P2002 ハンドリング追加（他店舗/platform コード衝突対策）→ 5-B-6a を Green 化。**メッセージは admin と同一の日本語「このクーポンコードは既に使用されています」へ統一**。`findFirst` 事前チェックは楽観的 UX 用に残すが、TOCTOU レースの**最終防御は P2002 捕捉**（DB `code @unique` が一意性 SSOT・[design.md](./design.md) §4.1 補足） | `refactor(coupon): add P2002 handling to seller upsertCoupon` |
...
- **回帰テスト必須観点**:
  ...
  - **seller `upsertCoupon` の P2002 フォールバック（5-B-6）**: `findFirst` 事前チェックを**すり抜ける並行挿入**（同一 `code` の TOCTOU レース）をシミュレートし、`coupon.ts` の `upsertCoupon` が Prisma **P2002**（`PrismaClientKnownRequestError.code === "P2002"`）経路に入ることを検証。アサート: (1) 統一日本語メッセージ「このクーポンコードは既に使用されています」が返る/throw される（AC-F3-2）、(2) `findFirst` 事前チェックは**楽観的 UX 用に残存**する一方、一意性の**最終防御は P2002 捕捉**である（DB `code @unique` が SSOT・[design §4.1](./design.md#41-新規-querysrcqueriescouponts-に追加) 補足）。テスト名・アサートに `upsertCoupon` / `coupon.ts` / `P2002` を含める。
