...
> **親行だけでは V-7d（子孫の追随）は守れない。** 上の議論は「商品の紐づけ」と
> 「子の作成」の競合、つまり*親*を共有する 2 者の話である。subtree の移動は
> **子孫の `path` を書き換える** read-modify-write なので、競合相手は親を共有しない ——
> subtree の根 X を動かす側は X の旧親・新親を掴み、X の子孫 D を subtree の外へ
> 動かす側は D の旧親（= subtree 内のノード）と新親を掴む。**両者は一度も同じ行で
> 出会わない。** 結果、前者が読んだ「D は X 配下」というスナップショットは、後者の
> コミット後に無効化されているのに、そのまま rebase された `path` として書き戻される。
> `parentId` は新しい親を指しているのに `path` は旧 subtree 配下、という矛盾が残り、
> `path` は導出列なのでこの状態から自力復帰できない（実測値は
> [`tests/integration/category-tree-write.test.ts`](../../../tests/integration/category-tree-write.test.ts)
> の Scenario 5 が固定している: 修正前は `parentId = z` / `path = "b/x/d"`）。
>
> したがって `upsertCategory` のロック集合は次の 3 種を **id 昇順の 1 つの順序体系で**
> 掴む:
>
> 1. **旧親・新親** —— リーフ強制（V-5）との直列化。上の議論のとおり
> 2. **自ノード自身** —— 「subtree を動かす側」と「その子孫を動かす側」を出会わせる。
>    後者は自ノードを掴むので、これが無いと両者が並走する
> 3. **移動時は全子孫** —— 掴んでから読み直す。掴む前に読んだ `path` は待っている間に
>    古くなる（READ COMMITTED）
>
> ロック取得は待ちを伴うため、**掴んだ後に対象を読み直す**こと。読み直して対象集合が
> 増えたら掴み直し、増えなくなるまで繰り返す（自ノードを掴んだ後は親が変わらないので
> 収束する）。収束しない場合は
...
| V-7d | 再親子化の後、**全子孫の `path` / `depth` が新しい親に追随**する（旧親のフィルタでヒットしなくなる） | Q1。`path` の取り残しは DB の値でなく検索結果でしか表面化しない |
| V-7e | 子孫を subtree の外へ動かす**並行トランザクション**があっても、`path` と `parentId` が矛盾しない | Q5。親だけを掴む実装では両者が同じ行で出会わない（Q5 の「親行だけでは V-7d を守れない」注記） |
