...
# マイグレーション履歴のベースライン設定
# ※ `bunx prisma migrate dev --name init_postgresql --create-only` で生成された
#    完全なタイムスタンプ付きフォルダ名（例: 20260222101357_init_postgresql）を指定する
bunx prisma migrate resolve --applied <タイムスタンプ付きフォルダ名>
```
...
移行ガイドのスコープ外です。設計ノートとロードマップは [`docs/architecture/saas-roadmap.md`](../architecture/saas-roadmap.md) を参照してください。
