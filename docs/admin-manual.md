...
### 8.3 データの連動削除（物理削除・カスケード削除の影響）
スキーマ定義上、いくつかの関連データには `ON DELETE CASCADE`（連動物理削除）が設定されています。
* **影響**: 例えば「カテゴリ」や「サブカテゴリ」を物理削除すると、それに紐づいていた「商品」などの関連データが意図せず削除される可能性があります。
* **店舗の削除について**: 「4.4 店舗の削除（論理削除 / ソフトデリート）」に記載の通り、店舗の削除は論理削除（`isDeleted` を `true` に設定し、`deletedAt` に削除日時を記録）です。そのため、店舗に関連する商品や注文データがデータベースから物理的に消去されることはありません。ただし、店舗が非表示（ソフトデリート）になることで、関連する商品も公開ページや検索結果から非表示になり、購入できなくなります。
* **運用策**: データを削除する前に、そのカテゴリにアクティブな商品や未完了の注文が存在しないかを必ず確認してください。物理削除は復旧が困難なため、可能な限り、ステータスを `DISABLED` や `BANNED` に変更して非公開化することを推奨します。
