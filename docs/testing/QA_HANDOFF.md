...
| テストファイル総数（ダッシュボード集計） | **194**（2026-07-31 の `bun run coverage:dashboard` が走査した件数。本ラウンドは新規テストファイルなしのため 194 で不変）。lcov エントリ 301 / マトリクス 18/80 セル (23%) も不変 |
