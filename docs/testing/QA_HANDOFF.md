...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1402** passed / 1405 total / 144 スイート（143 passed + 1 skipped suite）— 2026-06-17 `upsertCoupon` の cross-store/PLATFORM クーポン hijack IDOR を修正（upsert 前に対象行を `findUnique` し `storeId !== store.id` を拒否、PLATFORM=null も対象）+ IDOR 3 階層 (a)(c) 回帰テスト +2（`f6e75fd`–`505e13b`）：1400→1402 passed |
