...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1841** passed / 1844 total / **177 スイート**（176 passed + 1 skipped suite）。2026-08-03 実測（`bun run test -- --no-coverage`）。増減の経緯・実測履歴は [`COVERAGE_REPORT.md §7 履歴`](./COVERAGE_REPORT.md#7-履歴) |
...
| Playwright E2E（main） | **41 tests/browser**（17 files・3 ブラウザ計 **123**）。**11 スペック**（purchase-flow / seller-onboarding / payment-error / search-filter / mobile-responsive / platform-coupon / stock-decrement / country-selector / messages / layout-chrome / security-headers）。Clerk 依存 spec は `CLERK_SECRET_KEY` 未設定時に自動 skip。2026-08-04 実測: 全プロジェクト `bunx playwright test --list` が `Total: 123 tests in 17 files`、`--project=chromium` / `firefox` / `webkit` が**各 41**（3 ブラウザ計は掛け算ではなく実測値。projects は 3 つとも同一 `testDir` を走査するため各ブラウザで件数が一致する）。**2026-08-04 フルラン実測（plans 042/044 完了時・`bash scripts/e2e/run-local.sh`）: 83 passed / 3 failed / 37 skipped / flaky 0 / 5.8m**。failed 3 件は visual ベースライン陳腐化のみ（plan 043 の担当）で、**認証系の失敗はゼロ**。所要は従前ベースライン 25.5m から **5.8m** へ短縮（サインイン後ハングの除去でリトライ消費が消滅）。増減の経緯・実測履歴は [`COVERAGE_REPORT.md §7 履歴`](./COVERAGE_REPORT.md#7-履歴) |
| Playwright Visual | **2 スペック**（cart / checkout）。2026-07-11 実測: **3 テストともベースライン陳腐化で failed**（cart-empty は高さ 720→1071px。plan 043 で目視ゲート付き再撮影予定）。**2026-08-04 フルランでも同 3 件が failed のまま**（chromium のみ・各 2 リトライ込み）。これがフルランに残る唯一の failed 群であり、plan 043 が解消担当 |
...
| テストファイル総数（ダッシュボード集計） | **196** / lcov エントリ **302** / マトリクス 18/80 セル (23%)。2026-08-03 実測（`bun run coverage:dashboard` → `docs/coverage-dashboard.html` の `matrix-data`）。lcov 由来の値のみ `coverage/lcov.info` に依存するため **2026-08-01 の測定値**。増減の経緯・実測履歴は [`COVERAGE_REPORT.md §7 履歴`](./COVERAGE_REPORT.md#7-履歴) |
