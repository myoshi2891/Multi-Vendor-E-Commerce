...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1752** passed / 1755 total / 175 スイート（174 passed + 1 skipped suite）。2026-07-26 に CodeRabbit レビュー対応の回帰 +3（`useCartStore.test.ts` の「persist ラウンドトリップ」×3 — 永続化カートがリロード後に復元されることを、インメモリ状態を破棄してから `persist.rehydrate()` する形で検証。plan 005 の未検証ギャップを閉塞）。スイート・スナップショットは不変 |
