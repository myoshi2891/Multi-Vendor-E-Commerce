...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1841** passed / 1844 total / 177 スイート（176 passed + 1 skipped suite・**スイート数不変**）。**2026-08-03 実測で本行の値を訂正**（`bun run test -- --no-coverage` → `Tests: 3 skipped, 1841 passed, 1844 total`）。直前の記録は **1829 / 1832** で、実測と **12 件**乖離していた。plan 042 / 051 は E2E のみを触っており Jest テストを 1 件も追加していないため、この差分は**本セッション以前から蓄積していたドリフト**であって今回の作業による増加ではない（原因は未特定 — 統計を更新したラウンドのいずれかで実行せずに値を据え置いた可能性が高い）。スイート数と skip 数は記録と一致している。直前の記録: 2026-08-01 に CodeRabbit レビュー（**12 巡目**）対応の回帰 **+3**（内訳は実測: `scan-tests.test.ts` 21→**24**〔静的走査が**文字列リテラル・テンプレート・コメントの中身をコードと同じに扱っていた**件。走査対象コードをフィクスチャ文字列として持つファイルが実件数の数倍に膨らみ、**本ファイル自身が実行時 21 件に対しダッシュボード 81 件**という自己矛盾を起こしていた。`findMaskedSpans` で非コード範囲を列挙し、`BLOCK_PATTERN` / `EACH_PATTERN` / `SKIP_PATTERN` の一致位置がその範囲に入るなら捨てる。**リテラルを剥がした文字列は作らない** —— `it("title", fn)` のタイトル自体が文字列リテラルなので剥がすと宣言が壊れる。testCount 側 +1・hasSkip 側 +2〔誤検知の否定 1 + 「常に false」実装を弾く対の 1〕〕）。**11 巡目の内訳 2 箇所を実測で訂正**: (1) `webhooks/route.test.ts` は **20→21 ではなく 19→20**（`git show 4e4534d1:` の生 `it(` が 15、`5c1ec584` で 16。`it.each` 展開込みの実行時が 19→20。**+1 という差分は正しく、絶対値だけが 1 ずつ過大だった**）。(2) `scan-tests.test.ts` の 17→21 は実行時としては正しいが、**ダッシュボードは同時点で 81 を表示**しており本表と割れていた（原因は上記の文字列リテラル取り違え。12 巡目で解消）。直前: 2026-08-01 に CodeRabbit レビュー（**11 巡目**）対応の回帰 **+7**（内訳は実測: `webhooks/route.test.ts` 19→**20**〔`user.deleted` の id を `trim()` 後の値で検証しながら**未 trim の値**を絞り込みキーに渡していた件。`"  user_x  "` は検証を通過し、trim 後なら一致するユーザーの PII 秘匿・削除が 0 件ヒットのまま 200 で「成功」していた〕、`paypal.test.ts` 30→**32**〔capture 応答の相関検証が `purchase_units[0].custom_id ?? capture?.custom_id` で束ねられ、**`??` が最初の非 nullish で短絡**するため外側が一致すると capture 側が一度も検査されなかった件。capture オブジェクトこそ実際の資金移動を表すため、外側だけ自注文に相関し内側が別注文を指す応答が Paid 確定まで到達しうる。存在する custom_id の**全一致**を要求する形へ変更し、「どちらの位置に載っていてもよい」契約は別テストで維持を固定〕、`scan-tests.test.ts` 17→**21**〔`it.each(<識別子>)` を 0 件と数えていた静的走査の欠陥。同一ファイル const と `@/` import の単一ホップを解決する分岐を追加し、解決不能時は 0 のまま（過大計上しない fail-safe）・空 `it.each([])` の既存契約も回帰網に含めた〕）。直前: 2026-08-01 に SonarCloud 重複解消リファクタ + 負の配送料バグ修正の回帰 **+16**・**スイート +1**（`src/lib/order-settlement.ts` 新設のため）（内訳は実測: `src/lib/order-settlement.test.ts` 新規 **14**〔`hasOrderSettledAfterConflict`。確定/未確定ステータス・注文不在・`userId` で絞らないクエリ形状に加え、**抽出前は stripe/paypal 双方でテストが 1 件も無かった** `catch (reReadError)` 分岐を Error / 非 Error の両方で駆動。両テストとも再読を `mockResolvedValue` でしか駆動していなかったため、小さな新規ファイルに移すと `new_coverage >= 80%` を割る。新規ファイルのカバレッジは statements/branches/functions/lines すべて **100%**〕、`user.test.ts` 68→**70**〔ITEM 方式の配送料で個数がクランプされておらず `validQuantity === 0` に `fee - extra` = **負の配送料**が算出される件。`saveUserCart`（`Cart.total` 経路）と `placeOrder`（`OrderItem.shippingFee` 経路）の 2 経路を Red で実測〔`shippingFee: "7"`〕してから `Math.max(0, quantity - 1)` で Green 化〕）。直前: 2026-07-31 に CodeRabbit レビュー（**10 巡目**）対応の回帰 **+2**（内訳は実測: `paypal.test.ts` 29→**30**〔`fetch` はヘッダ受信で解決するため `clearTimeout` を直後に置くと**本文読み取りが予算の外**に出る件。本文が返らないまま予算ぶん時計を進めて signal が abort されることを固定〕、`user.test.ts` 67→**68**〔`placeOrder` の `$transaction` が Prisma 既定の maxWait 2s / timeout 5s に委ねられていた件。住所行の `FOR UPDATE` を保持する時間の上限でもあるため明示を要求〕）。直前: 2026-07-31 に CodeRabbit レビュー（9 巡目）対応の回帰 **+2**（内訳は実測: `paypal.test.ts` 27→**29**〔retrieve と capture が**単一の 10s タイマー / AbortController を共有**しており、retrieve が予算を食い潰すと capture が途中で abort され得た件。2 回の `fetch` が**異なる `signal` インスタンス**を受け取ること + retrieve 側の不一致 throw 後にタイマーが残らないことを固定〕。`user.test.ts` は 67 **不変** — 住所所有権 TOCTOU の閉塞（`findFirst` → `SELECT … FOR UPDATE`）に伴う既存 TOCTOU テストの**書き換え**であり、`$queryRaw` が `order.create` より前に呼ばれ 0 行なら throw する呼び出し契約へ差し替えた）。直前: 2026-07-30 に CodeRabbit レビュー（8 巡目）対応の回帰 **+9**（内訳は実測: `db-retry.test.ts` 16→**19**〔`it.each` 2 行（`2 ** 48` / `MAX_SAFE_INTEGER`）+ 上限内消化 1。**有限の**巨大値が素通しされ `randomInt` が `ERR_OUT_OF_RANGE` を投げて P2034 が化けていた — 7 巡目で閉じた `ERR_INVALID_ARG_TYPE` と同一欠陥クラスの残存〕、`stripe.test.ts` 39→**41**〔canceled 再作成キーの決定論性・同一 canceled を 2 回観測すると同一キー / 上限到達時の throw と canceled id を保存しないこと〕、`paypal.test.ts` 23→**27**〔capture **前**の `GET orders/{id}` 検証: `custom_id` / `amount` / `currency_code` の各不一致で capture が呼ばれないこと + 正常系の呼び出し順序固定〕）。スナップショットは不変 |
| カバレッジ全体（lcov 2026-08-01 実測・11 巡目後） | Statements **66.8%** / Branches **46.86%** / Functions **55.39%** / Lines **65.81%**（前回 66.8 / 46.78 / 55.34 / 65.8 — Statements ±0 / Branches +0.08 / Functions +0.05 / Lines +0.01。本ラウンドの実コード修正は既存関数内の分岐追加が主で新規関数を増やさないため、伸びは Branches に集中する） |
...
| Playwright E2E（main） | **11 スペック**（purchase-flow / seller-onboarding / payment-error / search-filter / mobile-responsive / platform-coupon / stock-decrement / **country-selector** 🆕 2026-08-03 plan 051 / messages / layout-chrome / **security-headers** 🆕 2026-07-18 plan 061）。Clerk 依存 spec は `CLERK_SECRET_KEY` 未設定時に自動 skip。**実行時値は 41 のまま不変（全ブラウザ 123 / 17 ファイル）**: plan 047 の un-skip は宣言済みテストを skip から active に変えるだけで総数を動かさない（動くのは skip / passed の内訳）。**plan 047 実測（2026-08-03・3 ブラウザ・`run-local.sh`）: `payment-error` + `platform-coupon` で 9 passed / 6 skipped / 0 failed / flaky 0**（従前は payment-error の住所未選択が 3 ブラウザとも skip、platform-coupon は間欠ハングで failed）。payment-error の残存 skip は `:58` 在庫切れ表示（機能未実装）と `:70` 二重送信（plan 006 依存）の 2 件 × 3 ブラウザのみ。**042 が「別事案」として残した間欠ハングは本ラウンドで根本原因を特定して除去した**（`waitForPostSignInSettle` を注文フロー spec から外す。詳細は冒頭の HEAD 注記）。**2026-07-11 初のフル実測（3 ブラウザ 111 テスト / `run-local.sh` + `--global-timeout=3600000` / 25.5m）: 52 passed / 17 failed / 39 skipped / 3 did not run** — 認証系の失敗は **13 件**（各 3 リトライ込みで wall-clock を押し上げた主因）。findings-16 の「16 instance」は 13 + 別カウント 3 の**合計**であって別の母数ではない —— 見積りの前提が読めなくなるため両者を混在させず、**13 件**に統一する（[`plans/044`](../../plans/044-e2e-run-guardrails.md) の根拠値定義に従う）。原因は signIn ヘルパーの Clerk UI ドリフト単一（**plan 042 で修復予定**。skip 39 の内訳 = 静的 18 + a11y/visual の chromium 限定 14 + firefox ローカルゲート 7。詳細: [`plans/audit/findings-16-e2e-coverage.md`](../../plans/audit/findings-16-e2e-coverage.md)）。**2026-07-18: `security-headers.spec.ts` を 3 ブラウザ実測で 6/6 pass**（Clerk 非依存・`request` フィクスチャのみ使用のため常時実行可能）。**2026-07-27 の「23→37 で一致」という記録は訂正済み（2026-07-30）。** `8637bca5` は `BLOCK_PATTERN` が `test.skip(` を拾わない欠陥（`it.each` 欠測 `c1be6d7` と同型）を修正したが、同時に**本体内の注釈形 `test.skip(cond, reason)` も 1 ケースとして計上**するようになっていた。注釈形は囲みの `test('title')` が既に計上済みなので**二重計上**であり、当時 37 に着地したのは**注釈形 16 件ぶんの過大計上がたまたま古い基準値 37 に一致した**結果で、真の値ではなかった。**実測（2026-08-03・plan 051 追加後）**: 実行時値は **41**（`bunx playwright test --list --project=chromium` の `Total: 41 tests in 17 files`。全ブラウザでは `Total: 123 tests in 17 files` = 41 × 3）。`country-selector.spec.ts` の 2 件ぶん増えている。**plan 042 実行後の実測（2026-08-03・chromium）**: 認証依存 spec の一括実行で **7 passed / 2 failed**。**失敗 2 件はいずれもサインイン起因ではない** —— `signInWithPassword` でのエラーは 0 件で、失敗時 DOM もサインイン済みヘッダー（`img "user name"`）を示しており、落ちているのは**その後**の商品ページ `goto`（`gotoStable` の 30s × 3 リトライがタイムアウト）である。これは `run-local.sh` ヘッダー記載の既知ハングと同じ症状で、**実行ごとに落ちるテストが移動する**（1 回目は stock-decrement のみ、2 回目は stock-decrement + platform-coupon）。**直前の実測（2026-07-30）**: 実行時値は **39**（`Total: 117 tests in 16 files` ÷ 3 ブラウザ）。`88f4eee5` で宣言形／注釈形を区別した後の静的値は **36**。残差 **3** は `for (const … of […]) { test(…) }` のループ生成ぶん（`layout-chrome` 静的 5 / 実行時 7 = +2、`security-headers` 静的 1 / 実行時 2 = +1）で、**静的走査の原理的限界**として追わずに明記する。なお `hasSkip`（ファイルに skip マーカーが存在するか）の意味は**据え置き** — 注釈形も skip マーカーではあり、変えると Skipped テスト統計という別 SSOT に波及するため |
...
| Playwright a11y | **4 スペック**（sign-in / seller-apply / checkout / profile）。**2026-08-03 実測（chromium）: 4 spec すべて passed**（plan 042 で解消）。内訳: sign-in の**実 WCAG 違反 svg-img-alt**（フッター SendIcon の `<svg role="img">` に代替テキスト無し / serious）は `c25a8768` で `aria-label` を付与して是正（同型の wishlist / order アイコンも予防的に是正）、checkout / profile の signIn ドリフトは `235754b8` の共有ヘルパー化で解消。直前: 2026-07-11 実測では seller-apply のみ passed だった |
| 型エラー | **0 件** |
...
| テストファイル総数（ダッシュボード集計） | **196**（2026-08-03 の `bun run coverage:dashboard` が走査した件数。生成物 `docs/coverage-dashboard.html` の `matrix-data` は `generatedAt: 2026-08-03T08:51:05.697Z` / `totalTestFiles: 196`）。plan 051 の `tests/e2e/country-selector.spec.ts` 新設ぶんで 195→196。直前は 2026-08-01 時点の **195**（`src/lib/order-settlement.test.ts` 新設ぶんで 194→195。11 巡目・12 巡目とも新規テストファイルなし）。lcov エントリ **302** / マトリクス 18/80 セル (23%)（lcov は `coverage/lcov.info` 依存のため **2026-08-01 の測定値のまま**で、2026-08-03 の再生成は同じ lcov を読み直しただけ）。**12 巡目でダッシュボードの過大計上 2 件を是正**: `scan-tests.test.ts` **81 → 24**（実行時 24 と一致。`hasSkip` も **true → false** に是正 —— フィクスチャ文字列内の `.skip` を根拠にヒートマップの `◐`〔partial〕判定へ流れていた）、`size.test.ts` **9 → 8**（`size.test.ts:144` の**コメントアウトされた** `it(` を宣言として数えていた）。**この 2 件以外にファイル単位の変動は無い**（再生成前後の `testCount` / `hasSkip` 全件 diff で確認）。**11 巡目で `order-settlement.test.ts` の表示が 6 → 14 cases に是正**（`scan-tests.ts` が `it.each(<識別子>)` を 0 件と数えていた欠陥。`it.each([...])` 欠測 `c1be6d7` と同型で、テーブルを名前付き定数へ括り出した形が丸ごと不可視だった） |
