...
> **最終更新**: 2026-09-02 / **HEAD**: `9b4ea311`（**plan 067 — カテゴリツリー Phase B（読み取りのサブツリー化）を実装中 / IN PROGRESS**。読み取りは新 FK `categoryNode` のサブツリー prefix へ切り替わり、旧 `?subCategory=` は 308 で正準化される。`8bebdc6e` `subtreeOf` / `10707b6d` slug を lowercase kebab へ制約（**実 DB 実測で違反 0 件**）/ `4cd23f1d` 別名フォールバック付き slug resolver / `168c4ea7` browse フィルタのサブツリー化 / `a1dbe5dc` `getAllCategories` のツリー化 + フィルタ UI 再帰化 / `8cca6a84` dual-write / `5c4b2501` Phase B 再同期マイグレーション + 統合 6 シナリオ / `c478ec54` 308 正準化 / `9b4ea311` prefix 境界の一元化。**残作業と BLOCKED は下の「次回着手用 依頼プロンプト」を参照**。直前は `31ab8c20`（未コミット作業の片付け 3 コミット）

---

## 現在の実装状態サマリ

### テスト統計（Jest は 2026-08-31 実測 / Integration は 2026-08-31 実測 / E2E の件数は 2026-08-31 実測・フルランは 2026-08-04 実測。lcov カバレッジは 2026-08-11 実測）
...
#### 067-B: カテゴリツリー Phase B の残作業（plan 067 / **IN PROGRESS**）

**現在地**: `9b4ea311`。読み取りのサブツリー化・dual-write・再同期マイグレーション・308 正準化は
**実装済みで緑**。残りは storefront の一部リンク、統合テスト 3 本、E2E 1 本、docs 同期、README 更新。

```
plans/067-implement-category-tree-queries.md の残作業を進めてください。

【完了済み（コミット 8bebdc6e〜9b4ea311）】
- src/lib/category-tree.ts: subtreeOf / isWithinSubtree / resolveCategoryNode / buildCategoryTree
- src/lib/schemas.ts: Category・SubCategory の slug を /^[a-z0-9]+(?:-[a-z0-9]+)*$/ へ制約
  （実 DB 実測で違反 0 件を確認済み。Store / OfferTag の url は path に載らないので据え置き）
- src/queries/product.ts: browse フィルタを categoryNode のサブツリーへ + dual-write
- src/queries/category.ts: getAllCategories をツリー返却へ（店舗スコープは祖先を prefix 展開）
- src/components/store/browse-page/filters/category/: 2 段固定 → 再帰
- src/app/(store)/browse/page.tsx: ?subCategory= を permanentRedirect（308）で正準化
- prisma/migrations/20260901223148_category_tree_phase_b_resync: PHASE_B_RESYNC 区間
- tests/integration/category-tree-resync.test.ts（6 シナリオ）+ setup/migration-sql.ts

【残作業】
1. storefront の残りリンク: src/components/store/layout/footer/links.tsx が
   ?subCategory= を生成している（308 を踏むだけで動作はするが 1 ホップ無駄）。
   ※ src/components/store/home/category-card.tsx は **変更しない** ——
     FeaturedCategoryType は home.ts の legacy subCategories 由来で、そこに載る url は
     リネーム前の旧 slug。?category= に変えると CATEGORY 別名では解決できず 0 件になる。
     ?subCategory= のままなら SUB_CATEGORY 別名で正しく解決される。
2. 統合テスト（tests/integration/product-browse.test.ts）:
   - V-1: subtreeOf("electronics/camera") が electronics/camera-accessories を含まない
   - V-6: 存在しない slug で 0 件（全件表示に化けない）
   - 3 階層目の商品が祖先カテゴリのフィルタでヒットする（ツリー化の本題）
   - dual-write: 商品作成後に旧 subCategoryId と新 categoryNodeId の両方が埋まる
3. E2E（tests/e2e/search-filter.spec.ts）V-2: 旧 ?subCategory= が 308 で正準ノードへ着地
4. spec-sync-after-test skill で docs 同期（別コミット）
5. plans/README.md の 067 行を DONE にし、実行記録を書く（別コミット）

【必ず守ること】
- 統合テストのコマンドは `bun run test:integration`。
  plan 067 本文の `bun run test -- tests/integration/...` は**動作しない**
  （jest.config.js が /tests/integration/ を除外しているため）。
- home.ts / size.ts / src/app/dashboard/** は 0 差分を維持（実測で確認済み）。
- prefix 境界は src/lib/category-tree.ts の subtreeOf / isWithinSubtree のみ。
```

**BLOCKED 1 件**: 実 DB（Neon dev）への `bunx prisma migrate deploy` が
**権限で拒否**されており、Phase B の再同期マイグレーションは**実 DB へ未適用**。
使い捨て PostgreSQL では 6 シナリオすべて緑。適用にはオペレーターの承認が要る。
なお 2026-09-02 時点の実測では `Product.categoryNodeId IS NULL` は **0 件**なので、
dev DB の現データに限れば読み取り切替は既に安全である（再同期は将来分の保険）。
