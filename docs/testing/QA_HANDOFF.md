...
**Step 5 以降の Phase C（不可逆）は未着手**で、着手前にオペレーター承認と
067 の再同期マイグレーション適用が要る（下の 067-B 参照）。

次セッションで着手する順:

1. **E2E `tests/e2e/admin-category-tree.spec.ts` を緑にする（未検証コミット `524ba258`）。**
   初回実行は `page.getByRole("combobox").first()` が**ダッシュボードのサイドバーにある
   ナビゲーション用 combobox** を掴んで失敗した（フォームではなくメニューが開いた）。
   `page.locator("form").getByRole("combobox")` へ修正済みだが**再実行していない**。

   ```bash
   bun run test:e2e:local -- tests/e2e/admin-category-tree.spec.ts --project=chromium
   ```

   緑になったら 3 ブラウザで回し、E2E 件数（65 → 66 tests/browser・29 → 30 files）を
   本ファイルの統計テーブルへ反映する。

2. **ドキュメント同期が未完**。本ファイル（SSOT）は更新済みだが、
   `07-testing.md` / `COVERAGE_REPORT.md` / `docs/PROGRESS.md` への統計伝播と
   `bun run coverage:dashboard` による `docs/coverage-dashboard.html` 再生成は
   **まだ行っていない**（`.claude/rules/02-tdd-step-commit.md` の同期義務が残っている）。
