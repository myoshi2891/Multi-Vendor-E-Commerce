...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1790** passed / 1793 total / 176 スイート（175 passed + 1 skipped suite・**スイート数は不変** — 本ラウンドは全て既存ファイルへの追加）。2026-07-30 に CodeRabbit レビュー（7 巡目）対応の回帰 **+4**（内訳は実測: `db-retry.test.ts` 13→**16**〔`it.each` 3 行・`baseDelayMs` の 小数/NaN/負値。小数は `randomInt` が `ERR_INVALID_ARG_TYPE` を投げ P2034 が化けていた〕、`paypal.test.ts` 22→**23**〔P2025 再読で未確定なら settled へ正規化しない — `stripe.test.ts` の同型テストの PayPal 版〕）。スナップショットは不変 |
| カバレッジ全体（lcov 2026-07-30 実測） | Statements **66.55%** / Branches **46.5%** / Functions **55%** / Lines **65.56%**（前回 66.53 / 46.43 / 55 / 65.53） |
