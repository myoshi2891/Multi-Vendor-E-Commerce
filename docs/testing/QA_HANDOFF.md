...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1786** passed / 1789 total / 176 スイート（175 passed + 1 skipped suite・**スイート数は不変** — 本ラウンドは全て既存ファイルへの追加）。2026-07-30 に CodeRabbit レビュー（6 巡目）対応の回帰 **+17**（内訳は実測: `webhooks/route.test.ts` 15→**19**〔`it.each` 4 行・`user.deleted` の id 欠落/空文字/空白/非文字列で 400 かつ tx 未開始〕、`db-retry.test.ts` 8→**13**〔`it.each` 4 行 + 単体 1・`maxAttempts` の 0/負値/NaN/小数〕、`admin/coupons/columns.test.tsx` 20→**25**、`scan-tests.test.ts` 15→**17**〔宣言形／注釈形の区別〕、`coupon.test.ts` 95→**96**）。スナップショットは不変 |
| カバレッジ全体（lcov 2026-07-30 実測） | Statements **66.53%** / Branches **46.43%** / Functions **55%** / Lines **65.53%**（前回 66.48 / 46.37 / 54.94 / 65.48） |
...
### 🔴 現在アクティブな残課題（優先度順・2026-07-28 時点） {#active-open-issues}
