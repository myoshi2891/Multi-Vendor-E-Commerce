...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1845** passed / 1848 total / **178 スイート**（177 passed + 1 skipped suite）。2026-08-04 実測（`bun run test`。plan 028 で `src/queries/country.test.ts` を新設し +4 テスト / +1 スイート）。増減の経緯・実測履歴は [`COVERAGE_REPORT.md §7 履歴`](./COVERAGE_REPORT.md#7-履歴) |
| カバレッジ全体（lcov 2026-08-04 実測・plan 028 後） | Statements **66.94%** (5795/8657) / Branches **46.94%** (2443/5204) / Functions **55.48%** (921/1660) / Lines **65.94%** (5212/7903)（前回 66.8 / 46.86 / 55.39 / 65.81 — Statements +0.14 / Branches +0.08 / Functions +0.09 / Lines +0.13。plan 028 の country.ts は 19 行の小モジュールなので全体への寄与は小さい） |
