...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1769** passed / 1772 total / 176 スイート（175 passed + 1 skipped suite）。2026-07-28 に CodeRabbit レビュー（5 巡目）対応の回帰 +8 / +1 スイート — `coupons/columns.test.tsx` +7（**新規スイート**。seller 編集モーダルの `getCoupon` reject が未処理で、ユーザー通知もモーダル終了も無かった欠陥。構造 2 + fetchData 正常系 1 + 異常系 4）、`scan-tests.test.ts` +1（`it.skip.each` / `test.only.each` が両パターンをすり抜けテーブル行数を丸ごと欠測する欠陥。fixture 5 件中 4 件が不可視だった）。`coupon.test.ts` は件数不変（既存 5 アサーションを部分一致→完全一致アンカーへ変更）。スナップショットは不変 |
| カバレッジ全体（lcov 2026-07-28 実測） | Statements 66.48% / Branches 46.37% / Functions 54.94% / Lines 65.48% |
...
| テストファイル総数（ダッシュボード集計） | **194**（2026-07-28 の `bun run coverage:dashboard` が走査した件数。`coupons/columns.test.tsx` 新規追加で 193→194）。lcov エントリ 301 / マトリクス 18/80 セル (23%) は不変 |
