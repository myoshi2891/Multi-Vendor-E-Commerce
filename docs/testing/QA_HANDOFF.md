...
#### R6: improve Round 6 Integration 深掘りギャップ解消（残り plan 039）🆕 2026-07-11 起票
...
**2026-08-23 時点で 036 / 037 / 038 が DONE、残るは 039（browse フィルタ — Prisma 6 回帰網）のみ**。
039 には**現挙動の characterization**（既知ギャップの固定）シナリオが含まれる —
期待値の反転条件はプラン本文の STOP conditions / Maintenance notes を参照。
**038 からの申し送り**: 一時 DDL で失敗注入する統合テストを書く場合、(1) 注入点は tx の
**後段**でなければ原子性の証拠にならない（冒頭で落とすと置換が未実行なだけで、
`$transaction` が無くても緑になる）、(2) 制約の DROP は ADD 直前と `finally` の**両方**で
`IF EXISTS` を付ける（`finally` 側を素の DROP にすると、ADD が落ちた経路で二次例外が
本来の失敗原因を置き換える）。CI 直列化要件は `maxWorkers: 1` と実行ごとの専用
testcontainers（ADR-004）で既に充足しており、ワークフロー変更は不要。

```
plans/039-integration-test-product-browse-filters.md を読んで、プラン記載のステップどおりに実行してください。
...
#### R9: improve Round 9 E2E 残余ギャップ解消（残り plan 054）🆕 2026-07-12 起票
...
特定した 6 件の実行プラン。**2026-08-23 時点で 051・052・053・055・056 が DONE、残るは 054 のみ**。
...
（**先行依存はすべて解除済み**なので BLOCKED 判定は不要だが、
プラン冒頭の Drift check は必ず実行すること。VRT は 043 の再撮影を土台にしており、
ベースライン画像の差し替えには目視ゲートが要る。
