...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1874** passed / 1877 total / **178 スイート**（177 passed + 1 skipped suite）。2026-08-04 実測（`bun run test`。plan 028 で `src/queries/country.test.ts` を新設し +4 テスト / +1 スイート、plan 029 で `profile.test.ts` を 34→63 に拡張し +29 テスト・スイート数不変）。増減の経緯・実測履歴は [`COVERAGE_REPORT.md §7 履歴`](./COVERAGE_REPORT.md#7-履歴) |
| カバレッジ全体（lcov 2026-08-04 実測・plans 028/029 後） | Statements **67.49%** (5843/8657) / Branches **47.48%** (2471/5204) / Functions **55.48%** (921/1660) / Lines **66.55%** (5260/7903)（前回 66.8 / 46.86 / 55.39 / 65.81 — Statements +0.69 / Branches +0.62 / Functions +0.09 / Lines +0.74。伸びの大半は plan 029（`profile.ts` を Branches 67.81%→**100%**）。plan 028 の country.ts は 19 行の小モジュールなので寄与は小さい。Functions が動かないのは両プランとも既存関数の分岐を埋める作業で新規関数を増やさないため） |
