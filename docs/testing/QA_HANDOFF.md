...
| テストファイル総数（ダッシュボード集計） | **195**（2026-08-01 の `bun run coverage:dashboard` が走査した件数。`src/lib/order-settlement.test.ts` 新設ぶんで 194→195。11 巡目・12 巡目とも新規テストファイルなしのため 195 で不変）。lcov エントリ **302** / マトリクス 18/80 セル (23%)。**12 巡目でダッシュボードの過大計上 2 件を是正**: `scan-tests.test.ts` **81 → 24**（実行時 24 と一致。`hasSkip` も **true → false** に是正 —— フィクスチャ文字列内の `.skip` を根拠にヒートマップの `◐`〔partial〕判定へ流れていた）、`size.test.ts` **9 → 8**（`size.test.ts:144` の**コメントアウトされた** `it(` を宣言として数えていた）。**この 2 件以外にファイル単位の変動は無い**（再生成前後の `testCount` / `hasSkip` 全件 diff で確認）。**11 巡目で `order-settlement.test.ts` の表示が 6 → 14 cases に是正**（`scan-tests.ts` が `it.each(<識別子>)` を 0 件と数えていた欠陥。`it.each([...])` 欠測 `c1be6d7` と同型で、テーブルを名前付き定数へ括り出した形が丸ごと不可視だった） |
