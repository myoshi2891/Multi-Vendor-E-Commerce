...
| 指標 | 値 |
|------|-----|
| Jest テスト総数 (unit/component) | **1754** passed / 1757 total / 175 スイート（174 passed + 1 skipped suite）。2026-07-26 に CodeRabbit レビュー（3 巡目）対応の回帰 +2（`stripe.test.ts` の「canceled 済み intent が返った場合は別の冪等キーで作り直す」/「canceled 以外の status では作り直さない」— 冪等キー固定により注文がその金額で恒久的に決済不能になる経路を閉塞）。スイート・スナップショットは不変 |
