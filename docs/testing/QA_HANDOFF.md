...
| テストファイル総数（ダッシュボード集計） | **195**（2026-08-01 の `bun run coverage:dashboard` が走査した件数。`src/lib/order-settlement.test.ts` 新設ぶんで 194→195。本 11 巡目は新規テストファイルなしのため 195 で不変）。lcov エントリ **302** / マトリクス 18/80 セル (23%)。**11 巡目で `order-settlement.test.ts` の表示が 6 → 14 cases に是正**（`scan-tests.ts` が `it.each(<識別子>)` を 0 件と数えていた欠陥。`it.each([...])` 欠測 `c1be6d7` と同型で、テーブルを名前付き定数へ括り出した形が丸ごと不可視だった） |
