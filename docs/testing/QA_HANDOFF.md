...
特定した 6 件の実行プラン。**2026-08-12 時点で 051・052・053 が DONE、残るは 054〜056**。
plan 042（Step 4 の svg-img-alt 修正を含む全体）と 043 がいずれも DONE のため、
**054（VRT 拡大）・055（カート引き継ぎ）の先行依存はすべて解除済み**で、
...
```
plans/055-e2e-guest-cart-login-handoff.md を読んで、プラン記載のステップどおりに実行してください。
ルール: 変更は In scope のファイルのみ。実行は bash scripts/e2e/run-local.sh を使う
