...
> **最終更新**: 2026-09-02 / **HEAD**: `0ed9502a`（**plan 067 — カテゴリツリー Phase B（読み取りのサブツリー化）**が **DONE**。読み取りは新 FK `categoryNode` のサブツリー prefix へ切り替わり、旧 `?subCategory=` は 308 で正準化される。`8bebdc6e` `subtreeOf` / `10707b6d` slug を lowercase kebab へ制約（**実 DB 実測で違反 0 件**）/ `4cd23f1d` 別名フォールバック付き slug resolver / `168c4ea7` browse フィルタのサブツリー化 / `a1dbe5dc` `getAllCategories` のツリー化 + フィルタ UI 再帰化 / `8cca6a84` dual-write / `5c4b2501` Phase B 再同期マイグレーション + 統合 6 シナリオ / `c478ec54` 308 正準化 / `9b4ea311` prefix 境界の一元化 / `d7769375` footer リンクを正準 slug へ / `171ac4fa` 統合 V-1・V-6・3 階層 / `c094f7d4` 統合 dual-write / `0ed9502a` E2E V-2（308）。**残る BLOCKED は実 DB への `prisma migrate deploy`（権限）1 件のみ**で、下の「067-B」節に記録した。直前は `31ab8c20`（未コミット作業の片付け 3 コミット）

---

## 現在の実装状態サマリ

### テスト統計（Jest は 2026-09-02 実測 / Integration は 2026-09-02 実測 / E2E の件数は 2026-09-02 実測・フルランは 2026-08-04 実測。lcov カバレッジは 2026-08-11 実測）
...
#### 067-B の残 BLOCKED: Phase B 再同期マイグレーションの実 DB 適用

**BLOCKED**: 実 DB（Neon dev）への `bunx prisma migrate deploy` が**権限で拒否**されており、
`20260901223148_category_tree_phase_b_resync` は**実 DB へ未適用**。使い捨て PostgreSQL では
6 シナリオすべて緑。適用にはオペレーターの承認が要る（`safe-migration` skill 経由）。
