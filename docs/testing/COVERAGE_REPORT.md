...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **169** (Jest unit/component 160 / Jest integration 2 / Playwright 7) — 2026-06-19 profile-messages Phase 2+3 で `src/queries/message.test.ts` + `src/components/store/profile/messages/{conversation-thread,messages-container}.test.tsx` を新規追加（157 → 160）。profile-settings Phase 1 で `tests/component/store/{user-menu,profile-sidebar,settings-page}.test.tsx` 追加（154 → 157）。Phase 4 で E2E `tests/e2e/stock-decrement.spec.ts` 追加（Playwright 6 → 7） |
| テスト総数 | **1553 unit/component passed** (3 skipped) + **17 integration** — 2026-06-19 時点（profile-messages Phase 2+3 完了: 6 server action の認可/IDOR 3 階層/冪等/`$transaction` +31、購入者 UI の 5 秒ポーリング component テスト +14、1508→1553、157→160 スイート）。残 3 skip は DB ゲートの idempotency suite |
