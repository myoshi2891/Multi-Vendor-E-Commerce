...
| テスト総数 | **1686 unit/component passed** (3 skipped) + **17 integration** — 2026-07-17 に CodeRabbit 指摘対応後の全スイートで実測。stripe capture は未完了 intent を `Pending` に写像し、`requires_payment_method` は `last_payment_error` ありの場合だけ `Failed`、初期状態は `Pending` とする。残 3 skip は DB ゲートの idempotency suite |
