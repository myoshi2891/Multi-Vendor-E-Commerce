...
> **生成日**: 2026-05-21（**最終更新**: 2026-07-16）/ **対応する成果物**: [`docs/coverage-dashboard.html`](./coverage-dashboard.html) ([生成元](../../scripts/coverage-dashboard/))
...
| テスト総数 | **1676 unit/component passed** (3 skipped) + **17 integration** — 2026-07-16 実測。plan 024 は `userCountry` cookie 書き込みの shape・JSON・長さ・projection を回帰テストで保護。スイート数は172のまま。残 3 skip は DB ゲートの idempotency suite |
...
| 2026-07-16 | **plan 024 完了**: 公開 `setUserCountryInCookies` route は読取り側と同じ `isCountry` ガードを使い、不正JSON・shape不一致・各フィールド100文字超を400で拒否する。cookieは4フィールドだけを投影して `Path=/` で書込み、余分なフィールドを永続化しない。必須6ケースへテストを更新し、実測を 1676 passed / 1679 total / 172 suites へ同期（`58a6bd5` / `8bd7bfd`）。 |
