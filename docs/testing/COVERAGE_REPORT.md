...
| テスト総数 | **1681 unit/component passed** (3 skipped) + **17 integration** — 2026-07-16 に plan 023/024 統合後の全スイートで実測。plan 023 は公開 `index-products` GET の `page` / `limit` 正規化を5件で回帰固定し、plan 024 は `userCountry` cookie 書き込みを6件で保護。残 3 skip は DB ゲートの idempotency suite |
