...
| テスト総数 | **1712 unit/component passed** (3 skipped) + **17 integration** — 2026-07-18 に plan 059（PayPal capture の金額/相関/通貨検証 + settled ガード。負系 5 ケースで paypal.test.ts 15→20）後の全スイートで実測。直前の plan 058 で coupon.test.ts 77→84。残 3 skip は DB ゲートの idempotency suite。**integration 17 はダッシュボードの `integration × queries` 集計とも一致**（`scan-tests.ts` が `it.each` を 0 件と数えていた欠陥を `c1be6d7` で解消し 14→17） |
