...
| テスト総数 | **1717 unit/component passed** (3 skipped) + **17 integration** — 2026-07-18 に improve Round 13 P1 全 4 プラン（057 next bump / 058 getCoupon IDOR / 059 PayPal capture 検証 / 060 クーポン server-side Zod 検証）完了後の全スイートで実測。058 で coupon.test.ts 77→84、059 で paypal.test.ts 15→20、060 で coupon.test.ts 84→89。残 3 skip は DB ゲートの idempotency suite。**integration 17 はダッシュボードの `integration × queries` 集計とも一致**（`scan-tests.ts` が `it.each` を 0 件と数えていた欠陥を `c1be6d7` で解消し 14→17） |
