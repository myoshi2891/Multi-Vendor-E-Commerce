...
| テスト総数 | **1667 unit/component passed** (3 skipped) + **17 integration** — 2026-07-16 実測。plan 003 は Stripe 権威値・intent 注文照合と配送先住所所有権の回帰を2件追加。直近記録からの合計は 1662→1667（累積の統計ドリフトも同期、スイート数は172のまま）。残 3 skip は DB ゲートの idempotency suite |
