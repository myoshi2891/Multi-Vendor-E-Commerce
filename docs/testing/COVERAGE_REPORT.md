...
| テスト総数 | **1407 unit/component passed** (3 skipped) + **17 integration** — 2026-06-17 時点（`applyCoupon` Decimal 演算エラー経路テスト +4: `.mul()/.div()/.add()/.sub()` prototype spy で catch ブロックを検証、1403→1407、144 スイート不変）。残 3 skip は DB ゲートの idempotency suite |
