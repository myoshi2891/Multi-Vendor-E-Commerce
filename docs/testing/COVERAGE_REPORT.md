...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **161** (Jest unit/component 153 / Jest integration 2 / Playwright 6) — 2026-06-18 販売者ダッシュボード Phase 2-C で `stock-status-badge.test.tsx` + `inventory/columns.test.tsx` 新規追加（151 → 153） |
| テスト総数 | **1443 unit/component passed** (3 skipped) + **17 integration** — 2026-06-18 時点（販売者ダッシュボード Phase 2-C UI: `stock-status-badge.test.tsx` +3 / `inventory/columns.test.tsx` +5、1435→1443、145→147 スイート）。残 3 skip は DB ゲートの idempotency suite |
