...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **165** (Jest unit/component 157 / Jest integration 2 / Playwright 6) — 2026-06-18 販売者ダッシュボード Phase 3-B で `store-stats-cards` / `store-recent-orders` / `store-top-products` の RTL テスト新規追加（154 → 157） |
| テスト総数 | **1496 unit/component passed** (3 skipped) + **17 integration** — 2026-06-18 時点（販売者ダッシュボード Phase 3-B 完了: F1 店舗ダッシュボード UI 3 コンポーネント + RTL +6、1490→1496、151→154 スイート）。残 3 skip は DB ゲートの idempotency suite |
