...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **172**（Jest unit/component 163 / Jest integration 2 / Playwright E2E メイン 7）— この小計は Jest + E2E メインのスコープであり、**Visual 2 + a11y 4 は別計上**（ダッシュボード `coverage-dashboard.html` が SSOT・全カテゴリ合算）。2026-06-21 Compare 機能で `src/compare-store/useCompareStore.test.ts` + `src/components/store/compare/compare-grid.test.tsx` を新規追加（161 → 163）。2026-06-20 profile-messages Phase 4 で `src/components/dashboard/seller/seller-messages-container.test.tsx` を新規追加（160 → 161）。2026-06-19 Phase 2+3 で `src/queries/message.test.ts` + `src/components/store/profile/messages/{conversation-thread,messages-container}.test.tsx`（157 → 160）。profile-settings Phase 1 で `tests/component/store/{user-menu,profile-sidebar,settings-page}.test.tsx`（154 → 157）。Phase 4(F3) で E2E `tests/e2e/stock-decrement.spec.ts`（Playwright 6 → 7） |
| テスト総数 | **1601 unit/component passed** (3 skipped) + **17 integration** — 2026-06-21 時点（Compare 機能実装: `useCompareStore.test.ts` +8 + `compare-grid.test.tsx` +2 = +10、1591→1601、161→163 スイート）。残 3 skip は DB ゲートの idempotency suite |
