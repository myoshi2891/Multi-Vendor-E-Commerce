...
| テスト総数 | **1689 unit/component passed** (3 skipped) + **17 integration** — 2026-07-17 に CodeRabbit 指摘対応 第3弾後の全スイートで実測。place-order は成功後の遷移中もガードを保持し、admin 注文フィルタの `page` は 10,000 にクランプする。残 3 skip は DB ゲートの idempotency suite |
