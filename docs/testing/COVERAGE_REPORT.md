...
| テスト総数 | **1403 unit/component passed** (3 skipped) + **17 integration** — 2026-06-17 時点（`applyCoupon` の TOCTOU レースを修正: 無条件 `cart.update` を `couponId=null` 条件付き `updateMany`（CAS）へ置換し `count===0` で既適用拒否 + 3 階層 (a)(b)(c) 回帰テスト +1：1402→1403、144 スイート不変）。残 3 skip は DB ゲートの idempotency suite |
