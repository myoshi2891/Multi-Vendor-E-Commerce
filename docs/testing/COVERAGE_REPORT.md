...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **159** (Jest unit/component 151 / Jest integration 2 / Playwright 6) — 2026-06-18 販売者ダッシュボード Phase 2 で `src/queries/inventory.test.ts` 新規追加（150 → 151） |
| テスト総数 | **1435 unit/component passed** (3 skipped) + **17 integration** — 2026-06-18 時点（販売者ダッシュボード Phase 2: `inventory.test.ts` 新規 +22 / `getStockStatus` 境界 +6、1407→1435、144→145 スイート）。残 3 skip は DB ゲートの idempotency suite |
