...
| テスト総数 | **1685 unit/component passed** (3 skipped) + **17 integration** — 2026-07-17 に CodeRabbit 指摘対応後の全スイートで実測。stripe capture が未完了 intent（`processing` / `requires_action` / `requires_confirmation` / `requires_capture`）を `Failed` ではなく `Pending` に写像するようになり、後続 webhook の `succeeded` と DB が矛盾しなくなった。残 3 skip は DB ゲートの idempotency suite |
