...
| テスト総数 | **1591 unit/component passed** (3 skipped) + **17 integration** — 2026-06-20 時点（SonarCloud QG 修復 PR #145: 購入者/販売者メッセージングコンテナの重複を共有フック/レイアウトへ抽出し、`message.ts`/コンテナ/`user-menu.tsx` のカバレッジを ~100% 分岐へ底上げ +31、1560→1591、161 スイート不変）。残 3 skip は DB ゲートの idempotency suite |
