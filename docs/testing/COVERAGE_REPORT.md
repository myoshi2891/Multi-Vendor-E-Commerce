...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **169** (Jest unit/component 160 / Jest integration 2 / Playwright 7) — 2026-06-19 profile-settings Phase 1 で `tests/component/store/{user-menu,profile-sidebar,settings-page}.test.tsx` を新規追加（157 → 160）。Phase 4 で E2E `tests/e2e/stock-decrement.spec.ts` 追加（Playwright 6 → 7） |
| テスト総数 | **1508 unit/component passed** (3 skipped) + **17 integration** — 2026-06-19 時点（profile-settings Phase 1 完了: Settings リンク回帰 + sidebar エントリ + settings page 描画 +3、1505→1508、154→157 スイート）。残 3 skip は DB ゲートの idempotency suite |
