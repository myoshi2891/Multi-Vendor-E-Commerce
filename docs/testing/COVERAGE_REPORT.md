...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **162** (Jest unit/component 154 / Jest integration 2 / Playwright 6) — 2026-06-18 販売者ダッシュボード Phase 3-A で `src/queries/store-dashboard.test.ts` 新規追加（153 → 154） |
| テスト総数 | **1490 unit/component passed** (3 skipped) + **17 integration** — 2026-06-18 時点（販売者ダッシュボード Phase 3-A 完了: `store-dashboard.ts` query 4 種 + `store-dashboard.test.ts` +39、1451→1490、150→151 スイート）。残 3 skip は DB ゲートの idempotency suite |
