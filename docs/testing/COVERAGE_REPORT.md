...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **166** (Jest unit/component 157 / Jest integration 2 / Playwright 7) — 2026-06-19 販売者ダッシュボード Phase 4 で E2E `tests/e2e/stock-decrement.spec.ts` 新規追加（Playwright 6 → 7）。Phase 4 の Jest +9 は既存 `user.test.ts` / `order.test.ts` への追加（ファイル数不変） |
| テスト総数 | **1505 unit/component passed** (3 skipped) + **17 integration** — 2026-06-19 時点（販売者ダッシュボード Phase 4 完了: F3 在庫減算 +3（`user.test.ts`）/ F3-5 在庫復元 +6（`order.test.ts`）、1496→1505、154 スイート不変）。残 3 skip は DB ゲートの idempotency suite |
