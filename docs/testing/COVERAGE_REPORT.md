...
| lcov エントリ数 | **301** (2026-07-28 の再生成時に `bun run coverage:dashboard` が parse した件数。本行はローカルの `coverage/lcov.info` に依存するため、生成物と同時にしか更新できない〔旧値 297 は 2026-07-18 時点で、以後の再生成で 300 になっていたのに本表だけ据え置かれていた〕。カバレッジ率の SSOT は [`QA_HANDOFF.md`](./QA_HANDOFF.md) の「lcov 2026-07-28 実測」行〔Statements 66.48% / Branches 46.37% / Functions 54.94% / Lines 65.48%〕。`coverage/lcov.info` は `.gitignore` 対象で git 管理外。再生成は `bun run test -- --coverage`) |
