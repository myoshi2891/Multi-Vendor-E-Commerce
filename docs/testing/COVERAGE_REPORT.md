...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **173 ファイル**（Jest unit/component 164 / Jest integration 2 / Playwright E2E メイン 7）— この小計は Jest + E2E メインのスコープであり、**Visual 2 + a11y 4 は別計上**（ダッシュボード `coverage-dashboard.html` が SSOT・全カテゴリ合算）。ファイル追加履歴: 2026-06-22 PR #147 で `product-card.test.tsx` 新規。2026-06-21 Compare 機能で `useCompareStore.test.ts` + `compare-grid.test.tsx` 新規。2026-06-20 で `seller-messages-container.test.tsx`、2026-06-19 で `message.test.ts` + `{conversation-thread,messages-container}.test.tsx`、profile-settings Phase 1 で `{user-menu,profile-sidebar,settings-page}.test.tsx`、Phase 4(F3) で E2E `stock-decrement.spec.ts` を追加 |
| Jest スイート総数 | **164 スイート**（163 passed + 1 skipped）— スイート数の遷移: 161 → 163（2026-06-21 Compare 2 ファイル）→ 164（2026-06-22 PR #147 product-card 1 ファイル）。2026-06-22 のレビュー指摘修正（compare 回帰テスト +4）は既存 2 スイートへの追加でスイート数は不変 |
| テスト総数 | **1617 unit/component passed** (3 skipped) + **17 integration** — 2026-06-22 時点（compare レビュー指摘修正: `compare-grid` 非 Error 分岐 +1 + `useCompareStore` 永続化契約 +3 = +4、1613→1617、スイート不変。直前 PR #147: +12、1601→1613、163→164 スイート）。残 3 skip は DB ゲートの idempotency suite |
