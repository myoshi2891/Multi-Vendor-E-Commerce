...
| テスト総数 | **1707 unit/component passed** (3 skipped) + **17 integration** — 2026-07-18 に plan 058（`getCoupon` を所有店舗にスコープする IDOR read 修正 + `getCouponAsAdmin` 新設。IDOR 3 階層回帰テストで coupon.test.ts 77→84）後の全スイートで実測。残 3 skip は DB ゲートの idempotency suite。**integration 17 はダッシュボードの `integration × queries` 集計とも一致**（`scan-tests.ts` が `it.each` を 0 件と数えていた欠陥を `c1be6d7` で解消し 14→17） |
