...
| 指標 | 値 |
|---|---|
| テストファイル総数 | **187 ファイル**（Jest unit/component 175 / Jest integration 2 / Playwright E2E メイン 10）— この小計は Jest + E2E メインのスコープであり、**Visual 2 + a11y 4 は別計上**（ダッシュボード `coverage-dashboard.html` が SSOT・全カテゴリ合算）。ファイル追加履歴: 2026-07-18 plan 061 で E2E `tests/e2e/security-headers.spec.ts` 新規（E2E メイン 9→10）。2026-06-26 track-order 機能で `tests/component/store/track-order-form.test.tsx` 新規（`trackOrder` ユニットは既存 `order.test.ts` に追加）。2026-06-22 PR #149 SonarCloud 修復で `src/components/store/static/content/content.test.ts` 新規。2026-06-22 support-forms 機能で `src/queries/support.test.ts` + `src/components/store/support/support-form.test.tsx` 新規。2026-06-22 storefront-static-pages 機能で `static-page-layout.test.tsx` + `about/page.test.tsx` + `customer-service/page.test.tsx` 新規。2026-06-22 offers 機能で `src/app/(store)/offers/page.test.tsx` 新規。2026-06-22 PR #147 で `product-card.test.tsx` 新規。2026-06-21 Compare 機能で `useCompareStore.test.ts` + `compare-grid.test.tsx` 新規。2026-06-20 で `seller-messages-container.test.tsx`、2026-06-19 で `message.test.ts` + `{conversation-thread,messages-container}.test.tsx`、profile-settings Phase 1 で `{user-menu,profile-sidebar,settings-page}.test.tsx`、Phase 4(F3) で E2E `stock-decrement.spec.ts` を追加 |
| Jest スイート総数 | **175 スイート**（174 passed + 1 skipped）— スイート数の遷移: 161 → 163（2026-06-21 Compare 2 ファイル）→ 164（2026-06-22 PR #147 product-card 1 ファイル）→ 165（2026-06-22 offers `offers/page.test.tsx` 1 ファイル）→ 168（2026-06-22 storefront-static-pages 3 ファイル）→ 170（2026-06-22 support-forms 2 ファイル）→ 171（2026-06-22 PR #149 `content/content.test.ts` 1 ファイル）→ 172（2026-06-26 track-order `track-order-form.test.tsx` 1 ファイル）→ 174（2026-07-16 plans 007-009 ログ集約 `src/lib/log.test.ts` + 二重送信ガード `place-order.test.tsx` の 2 ファイル）→ 175（2026-07-18 CodeRabbit Phase 1 P2034 再試行ヘルパー `src/lib/db-retry.test.ts` の 1 ファイル）。 |
| テスト総数 | **1761 unit/component passed** (3 skipped・175 スイート) + **17 integration** — 2026-07-27 に CodeRabbit レビュー対応（4 巡目）の回帰 +7（`coupon.test.ts` の検証・重複コードエラー文言の完全一致アンカー ×4〔seller / admin 両経路 + logError 不発火〕、`scan-tests.test.ts` の修飾子付きテスト計上 ×3〔+ 過大計上ガード 2 件〕。スイート不変）。その前、2026-07-26 に CodeRabbit レビュー対応（3 巡目）の回帰 +2（`stripe.test.ts` の canceled intent 再作成 ×2。スイート不変）。その前、同日 2 巡目の回帰 +3（`useCartStore.test.ts` の persist ラウンドトリップ ×3。スイート不変）。その前、同日 1 巡目の回帰 +3（`coupon.test.ts` の小数 discount 拒否・seller / admin 両経路 ×2、`stripe.test.ts` の P2025 判別 ×1。スイート不変）。その前、2026-07-24 に CodeRabbit セキュリティ修正の回帰 +2（`user.test.ts` placeOrder 住所所有権 TOCTOU / `webhooks/route.test.ts` SupportTicket PII 秘匿化。スイート不変）。その前、2026-07-18 に CodeRabbit ローカルレビュー Phase 1（ソース実バグ 5 件: Stripe 冪等キー / 決済 CAS / P2034 再試行 / `emptyCart()` 保護 / 空 `each` 計数）で +19・新スイート `src/lib/db-retry.test.ts`。その前に improve Round 13 P1 全 4 プラン（057 next bump / 058 getCoupon IDOR / 059 PayPal capture 検証 / 060 クーポン server-side Zod 検証）+ P2 2 プラン（061 レスポンス強化ヘッダ / 062 検索 route の error.message 漏洩停止）完了後の全スイートで実測。062 で `index-products/route.test.ts` 7→9（両ハンドラの 500 汎用化）。058 で coupon.test.ts 77→84、059 で paypal.test.ts 15→20、060 で coupon.test.ts 84→89。残 3 skip は DB ゲートの idempotency suite。**integration 17 はダッシュボードの `integration × queries` 集計とも一致**（`scan-tests.ts` が `it.each` を 0 件と数えていた欠陥を `c1be6d7` で解消し 14→17） |
| Jest スナップショット | **127** — 2026-05-28 時点（**B1+ 全完了** で 112 → 127 / 累計 49 プリミティブカバー） |
| マトリクスセル数 | **80** (8 カテゴリ × 10 ドメイン) |
| カバー済みセル | **18 / 80 (23%)** — 2026-07-26 ダッシュボード再生成時点（`coverage-dashboard.html` の `coveredCells` と一致。値は 2026-06-06 の是正時から変わっていないが、基準日は現行の生成物に合わせる。旧 `17/80 (21%)` はダッシュボードに対して未同期だったため是正済み） |
| lcov エントリ数 | **301** (2026-07-27 の再生成時に `bun run coverage:dashboard` が parse した件数。本行はローカルの `coverage/lcov.info` に依存するため、生成物と同時にしか更新できない〔旧値 297 は 2026-07-18 時点で、以後の再生成で 300 になっていたのに本表だけ据え置かれていた〕。カバレッジ率の SSOT は [`QA_HANDOFF.md`](./QA_HANDOFF.md) の「lcov 2026-07-27 実測」行〔Statements 66.18% / Branches 46.26% / Functions 54.69% / Lines 65.17%〕。`coverage/lcov.info` は `.gitignore` 対象で git 管理外。再生成は `bun run test -- --coverage`) |
