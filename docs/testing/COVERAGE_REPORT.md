...
| テスト総数 | **1400 unit/component passed** (3 skipped) + **17 integration** — 2026-06-17 時点（コードレビュー指摘対応: `updateCheckoutProductWithLatest` cross-cart IDOR 修正 + 回帰テスト +1、checkout `isDiscounted` の `isCouponCurrentlyValid` 追加、coupon.ts 3 関数の `requireStoreOwner` を try/catch 外へ移動：1399→1400 / 143→144 スイート）。残 3 skip は DB ゲートの idempotency suite |
