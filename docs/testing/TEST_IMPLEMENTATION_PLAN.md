...
> **優先度 P0〜P3 の定義**は
> [`QA_TEST_PERSPECTIVES.md` 「重要度の基準」](QA_TEST_PERSPECTIVES.md#重要度の基準)
> が SSOT（本計画書および Phase 5 / 6 が使う P3 = 顧客影響が未顕在の予防的強化・
> 設計先行を含む 4 段階）。
>
...
> **現行統計はハードコードしない**: テスト総数・スイート数・snapshot 数・skip 数の最新値は
> SSOT である [`QA_HANDOFF.md`](./QA_HANDOFF.md)「テスト統計」を参照すること。
> 上表は **2026-05-23 計画時点の Phase 別「目標」新規テスト数の内訳**（歴史的記録）であり、
> 実装の進捗に伴う累計値とは一致しない（過去の `990 テスト / 68 スイート / 40 snapshot` 表記は
> SSOT ドリフトの原因となったため撤去）。
