...
このプロジェクトには、543のユニットテスト（Jest）とE2Eテスト（Playwright）が実装されており、コミット前の品質チェックが必須です。
...
543のユニットテストが保証する高品質なコードベースを維持しながら、継続的な品質向上をサポートします。
