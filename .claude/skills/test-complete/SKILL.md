...
| 「ギャップ分析」「カバレッジギャップ」と指定あり | カバレッジ付きテスト + ギャップレポート |
...
### カバレッジギャップ（ギャップ分析モード時のみ）

| ファイル | Stmts | Branch | Func | Lines | 推奨 |
|---------|-------|--------|------|-------|------|
| `src/queries/XXX.ts` | 65% | 45% | 70% | 63% | P0: 分岐カバレッジ改善 |

**未テスト関数（export されているがテストなし）:**
- `src/queries/XXX.ts`: `functionA`, `functionB`

> テスト追加には `test-gen` スキルを使用してください（「XXX のテストを追加して」で実行可能）
