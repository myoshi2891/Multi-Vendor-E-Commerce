...
### CONDITIONAL（条件を満たせば許容）

| 操作 | 条件 |
|------|------|
| `it.skip` で個別テストを一時退避 | ①期限を設定（推奨: 2 週間以内に再着手） ②**同等カバレッジが他テストに存在する**ことを確認・明記 ③追跡 doc を 2 ヶ所更新（`QA_HANDOFF.md` の Open Issues + 関連 ADR の「後続調査」セクション） ④test ファイル内コメントで上記 3 リンクを明記 ⑤連続 3 サイクル以上の修正試行に失敗してから検討 |
| `describe.skip` で suite 全体を退避 | 上記に加えて: **suite 内の他テストでカバーできない領域がある**ことを明記し、本当に suite ごと退避すべきか個別 `it.skip` の集合の方が良いかを天秤にかける |

**実例**: `modal-provider.test.tsx:95` の [P1] setOpen テスト（OI-8）。詳細は [ADR-003 後続調査](../../../docs/architecture/decisions/003-modal-setopen-sync-for-react19.md#後続調査と一時スキップ判断)。
...
2. **エラー本文が空でも assertion 失敗と仮定**して対応 → 真因（React 19 strict act mode の cleanup 段階エラー の可能性）を見落とした
...
5. **Provider 同期化 (`9b77c59`) でも 1 サイクル偶発グリーンを「修正成功」と誤認**（同じ判断ミスの再発） → 次の commit `9040dcc`（docs only）で再失敗 → ADR-003 Status を `Accepted` → `Partial Mitigation` に訂正
6. **最終的に 6 仮説（A〜F）が未検証のまま該当テスト 1 件を `it.skip` で一時退避**（OI-8）。CI 安定優先＋同等カバレッジ残存を確認した上での妥協

本スキルはこの教訓を **次回のオペレータが踏まないため** の手順書:

- Step 1〜2 の事実確定を飛ばして Step 5 の code 修正に進まない
- 「1 サイクル両グリーン = 修正完了」と即断しない（**連続 N サイクルを基準とする**）
- 「エラー本文が空」は assertion failure ではないシグナル（分岐 D へ）
- 試行が複数回失敗したら **設計改善は別評価軸として残し、CI 安定は `it.skip` で優先**する判断もアリ。ただし: ①期限 ②同等カバレッジ確認 ③追跡 doc（QA_HANDOFF.md + ADR 後続調査セクション）の 3 条件を満たすこと

進行中の例: [ADR-003 後続調査と一時スキップ判断](../../../docs/architecture/decisions/003-modal-setopen-sync-for-react19.md#後続調査と一時スキップ判断) と [QA_HANDOFF.md OI-8](../../../docs/testing/QA_HANDOFF.md)
