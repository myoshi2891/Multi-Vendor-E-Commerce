...
const parseMoneyToCents = (text: string): number => {
    // 金額トークンは 1 つだけ許す。複数あると「不正な 1 つ目を飛ばして
    // 2 つ目の正常値を返す」経路ができ、fail-fast が成立しない。
...
>
> **正規表現だけでは fail-fast にならない。** `String.prototype.match` は最初に
> 一致した位置を返すため、不正なトークンには「throw」ではなく「読み飛ばし」で
> 反応する —— `"$1,23.45 $12.34"` は 1 つ目を素通りして `1234` を返す。
> 検算対象は 1 つの表示値なので、**先に `$` の個数を 1 に固定**して、
