import type { SeedReview } from "../types";

const img = "/assets/images/no_image.png";

/** レビューデータ */
export const SEED_REVIEWS: SeedReview[] = [
    {
        variant: "Classic Black",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 4,
        color: "Black",
        size: "L",
        quantity: "1",
        likes: 11,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-noir-cashmere-double-breasted-coat",
        images: [],
    },
    {
        variant: "Ivory",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Ivory",
        size: "XS",
        quantity: "1",
        likes: 2,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-noir-cashmere-double-breasted-coat",
        images: [],
    },
    {
        variant: "Classic Black",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 5,
        color: "Black",
        size: "XS",
        quantity: "2",
        likes: 25,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-noir-cashmere-double-breasted-coat",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Midnight Black",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 4,
        color: "Midnight Black",
        size: "6",
        quantity: "1",
        likes: 33,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-noir-silk-charmeuse-evening-dress",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Midnight Black",
        review: "Exquisite design, looks much more expensive than it actually is.",
        rating: 5,
        color: "Midnight Black",
        size: "8",
        quantity: "1",
        likes: 47,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-noir-silk-charmeuse-evening-dress",
        images: [],
    },
    {
        variant: "Midnight Black",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 5,
        color: "Midnight Black",
        size: "8",
        quantity: "2",
        likes: 33,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-noir-silk-charmeuse-evening-dress",
        images: [],
    },
    {
        variant: "Charcoal Grey",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 4,
        color: "Charcoal Grey",
        size: "50",
        quantity: "1",
        likes: 27,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-noir-italian-wool-three-piece-suit",
        images: [],
    },
    {
        variant: "Charcoal Grey",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 4,
        color: "Charcoal Grey",
        size: "54",
        quantity: "2",
        likes: 15,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-noir-italian-wool-three-piece-suit",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Charcoal Grey",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Charcoal Grey",
        size: "50",
        quantity: "2",
        likes: 48,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-noir-italian-wool-three-piece-suit",
        images: [],
    },
    {
        variant: "Charcoal Grey",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 4,
        color: "Charcoal Grey",
        size: "52",
        quantity: "2",
        likes: 29,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-noir-italian-wool-three-piece-suit",
        images: [],
    },
    {
        variant: "Black",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 5,
        color: "Black",
        size: "XS",
        quantity: "1",
        likes: 34,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-noir-merino-wool-turtleneck",
        images: [],
    },
    {
        variant: "Cream",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 5,
        color: "Cream",
        size: "XS",
        quantity: "2",
        likes: 2,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-noir-merino-wool-turtleneck",
        images: [],
    },
    {
        variant: "Black",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 5,
        color: "Black",
        size: "S",
        quantity: "2",
        likes: 39,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-noir-merino-wool-turtleneck",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Grey Flannel",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 4,
        color: "Grey",
        size: "8",
        quantity: "2",
        likes: 25,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-noir-tailored-wide-leg-trousers",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black Crepe",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 5,
        color: "Black",
        size: "12",
        quantity: "2",
        likes: 2,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-noir-tailored-wide-leg-trousers",
        images: [],
    },
    {
        variant: "Grey Flannel",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 4,
        color: "Grey",
        size: "2",
        quantity: "2",
        likes: 3,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-noir-tailored-wide-leg-trousers",
        images: [],
    },
    {
        variant: "Black Crepe",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 5,
        color: "Black",
        size: "10",
        quantity: "2",
        likes: 44,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-noir-tailored-wide-leg-trousers",
        images: [],
    },
    {
        variant: "Black Calfskin",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 4,
        color: "Black",
        size: "40",
        quantity: "1",
        likes: 8,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-noir-leather-chelsea-boots",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black Calfskin",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 4,
        color: "Black",
        size: "42",
        quantity: "2",
        likes: 18,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-noir-leather-chelsea-boots",
        images: [],
    },
    {
        variant: "Black Calfskin",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 4,
        color: "Black",
        size: "41",
        quantity: "1",
        likes: 37,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-noir-leather-chelsea-boots",
        images: [],
    },
    {
        variant: "Brown Suede",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 5,
        color: "Brown",
        size: "40",
        quantity: "1",
        likes: 25,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-noir-leather-chelsea-boots",
        images: [],
    },
    {
        variant: "Brown Suede",
        review: "Exquisite design, looks much more expensive than it actually is.",
        rating: 5,
        color: "Brown",
        size: "36",
        quantity: "2",
        likes: 12,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-noir-leather-chelsea-boots",
        images: [],
    },
    {
        variant: "Navy",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 5,
        color: "Navy",
        size: "S",
        quantity: "2",
        likes: 34,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-maison-embroidered-cocktail-dress",
        images: [],
    },
    {
        variant: "Navy",
        review: "Exquisite design, looks much more expensive than it actually is.",
        rating: 5,
        color: "Navy",
        size: "M",
        quantity: "1",
        likes: 42,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-maison-embroidered-cocktail-dress",
        images: [],
    },
    {
        variant: "Burgundy",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 4,
        color: "Burgundy",
        size: "M",
        quantity: "2",
        likes: 22,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-maison-embroidered-cocktail-dress",
        images: [],
    },
    {
        variant: "Navy",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Navy",
        size: "M",
        quantity: "1",
        likes: 30,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-maison-embroidered-cocktail-dress",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Ivory",
        review: "Fantastic fit and finish. Very pleased with my purchase.",
        rating: 5,
        color: "Ivory",
        size: "XS",
        quantity: "2",
        likes: 24,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-maison-silk-organza-blouse",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Ivory",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 5,
        color: "Ivory",
        size: "XL",
        quantity: "2",
        likes: 11,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-maison-silk-organza-blouse",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Ivory",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 4,
        color: "Ivory",
        size: "S",
        quantity: "2",
        likes: 14,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-maison-silk-organza-blouse",
        images: [],
    },
    {
        variant: "Black",
        review: "The packaging was lovely, and the item itself is even better.",
        rating: 5,
        color: "Black",
        size: "40",
        quantity: "2",
        likes: 38,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-maison-tailored-blazer",
        images: [],
    },
    {
        variant: "Navy",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 3,
        color: "Navy",
        size: "34",
        quantity: "1",
        likes: 33,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-maison-tailored-blazer",
        images: [],
    },
    {
        variant: "Black",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Black",
        size: "34",
        quantity: "2",
        likes: 47,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-maison-tailored-blazer",
        images: [],
    },
    {
        variant: "Black",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 4,
        color: "Black",
        size: "M",
        quantity: "2",
        likes: 5,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-maison-pleated-midi-skirt",
        images: [],
    },
    {
        variant: "Black",
        review: "Exquisite design, looks much more expensive than it actually is.",
        rating: 5,
        color: "Black",
        size: "M",
        quantity: "1",
        likes: 39,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-maison-pleated-midi-skirt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 5,
        color: "Black",
        size: "L",
        quantity: "2",
        likes: 26,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-maison-pleated-midi-skirt",
        images: [],
    },
    {
        variant: "Dusty Rose",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 5,
        color: "Dusty Rose",
        size: "L",
        quantity: "1",
        likes: 15,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-maison-pleated-midi-skirt",
        images: [],
    },
    {
        variant: "Black",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Black",
        size: "XS",
        quantity: "1",
        likes: 29,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-maison-pleated-midi-skirt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Charcoal",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 4,
        color: "Charcoal",
        size: "M",
        quantity: "2",
        likes: 33,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-maison-cashmere-wrap-coat",
        images: [],
    },
    {
        variant: "Charcoal",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Charcoal",
        size: "L",
        quantity: "1",
        likes: 31,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-maison-cashmere-wrap-coat",
        images: [],
    },
    {
        variant: "Camel",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 5,
        color: "Camel",
        size: "L",
        quantity: "2",
        likes: 50,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-maison-cashmere-wrap-coat",
        images: [],
    },
    {
        variant: "Charcoal",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 5,
        color: "Charcoal",
        size: "L",
        quantity: "1",
        likes: 44,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-maison-cashmere-wrap-coat",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Charcoal",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "Charcoal",
        size: "XL",
        quantity: "1",
        likes: 29,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-maison-cashmere-wrap-coat",
        images: [],
    },
    {
        variant: "Cognac",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 4,
        color: "Cognac",
        size: "One Size",
        quantity: "2",
        likes: 32,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-maison-leather-shoulder-bag",
        images: [],
    },
    {
        variant: "Cognac",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 4,
        color: "Cognac",
        size: "One Size",
        quantity: "2",
        likes: 43,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-maison-leather-shoulder-bag",
        images: [],
    },
    {
        variant: "Cognac",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "Cognac",
        size: "One Size",
        quantity: "2",
        likes: 12,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-maison-leather-shoulder-bag",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 4,
        color: "Black",
        size: "One Size",
        quantity: "2",
        likes: 48,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-maison-leather-shoulder-bag",
        images: [],
    },
    {
        variant: "18K Yellow Gold",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 5,
        color: "Yellow Gold",
        size: "45cm",
        quantity: "1",
        likes: 47,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-atelier-artisan-gold-chain-necklace",
        images: [],
    },
    {
        variant: "18K Yellow Gold",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 5,
        color: "Yellow Gold",
        size: "50cm",
        quantity: "1",
        likes: 14,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-atelier-artisan-gold-chain-necklace",
        images: [],
    },
    {
        variant: "18K Yellow Gold",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 4,
        color: "Yellow Gold",
        size: "45cm",
        quantity: "2",
        likes: 6,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-atelier-artisan-gold-chain-necklace",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "White Gold Pave",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 4,
        color: "White Gold",
        size: "8",
        quantity: "2",
        likes: 18,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-atelier-diamond-pave-ring",
        images: [],
    },
    {
        variant: "White Gold Pave",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 5,
        color: "White Gold",
        size: "6",
        quantity: "1",
        likes: 42,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-atelier-diamond-pave-ring",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "White Gold Pave",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "White Gold",
        size: "7",
        quantity: "2",
        likes: 10,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-atelier-diamond-pave-ring",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "White Gold Pave",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 4,
        color: "White Gold",
        size: "5",
        quantity: "1",
        likes: 48,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-atelier-diamond-pave-ring",
        images: [],
    },
    {
        variant: "White Gold Pave",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 4,
        color: "White Gold",
        size: "6",
        quantity: "1",
        likes: 34,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-atelier-diamond-pave-ring",
        images: [],
    },
    {
        variant: "Oxidized Sterling Silver",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 5,
        color: "Oxidized Silver",
        size: "M (17cm)",
        quantity: "1",
        likes: 12,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-atelier-hammered-silver-cuff",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Oxidized Sterling Silver",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 5,
        color: "Oxidized Silver",
        size: "M (17cm)",
        quantity: "1",
        likes: 7,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-atelier-hammered-silver-cuff",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Oxidized Sterling Silver",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 4,
        color: "Oxidized Silver",
        size: "L (19cm)",
        quantity: "2",
        likes: 34,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-atelier-hammered-silver-cuff",
        images: [],
    },
    {
        variant: "White Pearl Gold",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "White Pearl",
        size: "One Size",
        quantity: "2",
        likes: 6,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-atelier-pearl-drop-earrings",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "White Pearl Gold",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 3,
        color: "White Pearl",
        size: "One Size",
        quantity: "1",
        likes: 17,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-atelier-pearl-drop-earrings",
        images: [],
    },
    {
        variant: "White Pearl Gold",
        review: "Fantastic fit and finish. Very pleased with my purchase.",
        rating: 5,
        color: "White Pearl",
        size: "One Size",
        quantity: "2",
        likes: 6,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-atelier-pearl-drop-earrings",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Cognac Brown",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 4,
        color: "Cognac Brown",
        size: "One Size",
        quantity: "1",
        likes: 30,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-atelier-leather-portfolio-briefcase",
        images: [],
    },
    {
        variant: "Black Edition",
        review: "Fantastic fit and finish. Very pleased with my purchase.",
        rating: 4,
        color: "Black",
        size: "One Size",
        quantity: "1",
        likes: 3,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-atelier-leather-portfolio-briefcase",
        images: [],
    },
    {
        variant: "Black Edition",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Black",
        size: "One Size",
        quantity: "1",
        likes: 44,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-atelier-leather-portfolio-briefcase",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black Edition",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 4,
        color: "Black",
        size: "One Size",
        quantity: "1",
        likes: 5,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-atelier-leather-portfolio-briefcase",
        images: [],
    },
    {
        variant: "Garden Botanica",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 4,
        color: "Multi (Green/Gold/Burgundy)",
        size: "90cm x 90cm",
        quantity: "1",
        likes: 5,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-atelier-hand-painted-silk-scarf",
        images: [],
    },
    {
        variant: "Garden Botanica",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 3,
        color: "Multi (Green/Gold/Burgundy)",
        size: "90cm x 90cm",
        quantity: "1",
        likes: 12,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-atelier-hand-painted-silk-scarf",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Garden Botanica",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Multi (Green/Gold/Burgundy)",
        size: "90cm x 90cm",
        quantity: "1",
        likes: 48,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-atelier-hand-painted-silk-scarf",
        images: [],
    },
    {
        variant: "Burgundy",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 5,
        color: "Burgundy",
        size: "Medium",
        quantity: "1",
        likes: 15,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-velvet-quilted-leather-shoulder-bag",
        images: [],
    },
    {
        variant: "Black",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 5,
        color: "Black",
        size: "Small",
        quantity: "2",
        likes: 45,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-velvet-quilted-leather-shoulder-bag",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "Black",
        size: "Medium",
        quantity: "1",
        likes: 47,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-velvet-quilted-leather-shoulder-bag",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Black",
        size: "Small",
        quantity: "2",
        likes: 19,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-velvet-quilted-leather-shoulder-bag",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "The packaging was lovely, and the item itself is even better.",
        rating: 5,
        color: "Black",
        size: "Small",
        quantity: "1",
        likes: 38,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-velvet-quilted-leather-shoulder-bag",
        images: [],
    },
    {
        variant: "Black Suede",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "Black",
        size: "36",
        quantity: "2",
        likes: 29,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-velvet-suede-ankle-boots",
        images: [],
    },
    {
        variant: "Black Suede",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 5,
        color: "Black",
        size: "41",
        quantity: "1",
        likes: 41,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-velvet-suede-ankle-boots",
        images: [],
    },
    {
        variant: "Black Suede",
        review: "Fantastic fit and finish. Very pleased with my purchase.",
        rating: 4,
        color: "Black",
        size: "36",
        quantity: "2",
        likes: 11,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-velvet-suede-ankle-boots",
        images: [],
    },
    {
        variant: "Taupe Suede",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 5,
        color: "Taupe",
        size: "38",
        quantity: "2",
        likes: 21,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-velvet-suede-ankle-boots",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Ruby",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Ruby",
        size: "One Size",
        quantity: "1",
        likes: 22,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-velvet-silk-evening-clutch",
        images: [],
    },
    {
        variant: "Sapphire",
        review: "The packaging was lovely, and the item itself is even better.",
        rating: 4,
        color: "Sapphire",
        size: "One Size",
        quantity: "1",
        likes: 22,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-velvet-silk-evening-clutch",
        images: [],
    },
    {
        variant: "Emerald",
        review: "The packaging was lovely, and the item itself is even better.",
        rating: 4,
        color: "Emerald",
        size: "One Size",
        quantity: "1",
        likes: 15,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-velvet-silk-evening-clutch",
        images: [],
    },
    {
        variant: "Nude Patent",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 3,
        color: "Nude",
        size: "37",
        quantity: "2",
        likes: 21,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-velvet-patent-leather-pumps",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Nude Patent",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Nude",
        size: "40",
        quantity: "2",
        likes: 20,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-velvet-patent-leather-pumps",
        images: [],
    },
    {
        variant: "Black Patent",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 5,
        color: "Black",
        size: "38",
        quantity: "2",
        likes: 23,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-velvet-patent-leather-pumps",
        images: [],
    },
    {
        variant: "Black Patent",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 5,
        color: "Black",
        size: "36",
        quantity: "1",
        likes: 8,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-velvet-patent-leather-pumps",
        images: [],
    },
    {
        variant: "Ivory",
        review: "The packaging was lovely, and the item itself is even better.",
        rating: 4,
        color: "Ivory",
        size: "One Size",
        quantity: "1",
        likes: 28,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-velvet-structured-tote-bag",
        images: [],
    },
    {
        variant: "Camel",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 5,
        color: "Camel",
        size: "One Size",
        quantity: "1",
        likes: 1,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-velvet-structured-tote-bag",
        images: [],
    },
    {
        variant: "Ivory",
        review: "Fantastic fit and finish. Very pleased with my purchase.",
        rating: 5,
        color: "Ivory",
        size: "One Size",
        quantity: "2",
        likes: 34,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-velvet-structured-tote-bag",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Camel",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 5,
        color: "Camel",
        size: "One Size",
        quantity: "2",
        likes: 22,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-velvet-structured-tote-bag",
        images: [],
    },
    {
        variant: "Silver Buckle",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 3,
        color: "Silver",
        size: "85cm",
        quantity: "1",
        likes: 2,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-velvet-embossed-leather-belt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Silver Buckle",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 4,
        color: "Silver",
        size: "85cm",
        quantity: "2",
        likes: 19,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-velvet-embossed-leather-belt",
        images: [],
    },
    {
        variant: "Silver Buckle",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 5,
        color: "Silver",
        size: "90cm",
        quantity: "2",
        likes: 10,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-velvet-embossed-leather-belt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Gold Buckle",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 4,
        color: "Gold",
        size: "85cm",
        quantity: "1",
        likes: 10,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-velvet-embossed-leather-belt",
        images: [],
    },
    {
        variant: "Leather Strap",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 5,
        color: "Brown Leather",
        size: "One Size",
        quantity: "2",
        likes: 14,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-oro-chronograph-dress-watch",
        images: [],
    },
    {
        variant: "Leather Strap",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 5,
        color: "Brown Leather",
        size: "One Size",
        quantity: "2",
        likes: 0,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-oro-chronograph-dress-watch",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Steel Bracelet",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 4,
        color: "Steel",
        size: "One Size",
        quantity: "1",
        likes: 47,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-oro-chronograph-dress-watch",
        images: [],
    },
    {
        variant: "Black",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 3,
        color: "Black",
        size: "44",
        quantity: "1",
        likes: 22,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-oro-italian-leather-loafers",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "Fantastic fit and finish. Very pleased with my purchase.",
        rating: 5,
        color: "Black",
        size: "42",
        quantity: "2",
        likes: 33,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-oro-italian-leather-loafers",
        images: [],
    },
    {
        variant: "Cognac",
        review: "The packaging was lovely, and the item itself is even better.",
        rating: 5,
        color: "Cognac",
        size: "42",
        quantity: "1",
        likes: 32,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-oro-italian-leather-loafers",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Classic Collection",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "Multi",
        size: "One Size",
        quantity: "1",
        likes: 30,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-oro-silk-pocket-square-set",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Classic Collection",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 5,
        color: "Multi",
        size: "One Size",
        quantity: "1",
        likes: 3,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-oro-silk-pocket-square-set",
        images: [],
    },
    {
        variant: "Classic Collection",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Multi",
        size: "One Size",
        quantity: "2",
        likes: 1,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-oro-silk-pocket-square-set",
        images: [],
    },
    {
        variant: "Classic Collection",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 5,
        color: "Multi",
        size: "One Size",
        quantity: "1",
        likes: 2,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-oro-silk-pocket-square-set",
        images: [],
    },
    {
        variant: "Classic Collection",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 4,
        color: "Multi",
        size: "One Size",
        quantity: "2",
        likes: 2,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-oro-silk-pocket-square-set",
        images: [],
    },
    {
        variant: "Navy",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 5,
        color: "Navy",
        size: "52",
        quantity: "1",
        likes: 45,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-oro-cashmere-overcoat",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Charcoal",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 5,
        color: "Charcoal",
        size: "50",
        quantity: "2",
        likes: 8,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-oro-cashmere-overcoat",
        images: [],
    },
    {
        variant: "Charcoal",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 3,
        color: "Charcoal",
        size: "54",
        quantity: "2",
        likes: 21,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-oro-cashmere-overcoat",
        images: [],
    },
    {
        variant: "Charcoal",
        review: "The packaging was lovely, and the item itself is even better.",
        rating: 5,
        color: "Charcoal",
        size: "48",
        quantity: "2",
        likes: 40,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-oro-cashmere-overcoat",
        images: [],
    },
    {
        variant: "Navy",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Navy",
        size: "52",
        quantity: "2",
        likes: 28,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-oro-cashmere-overcoat",
        images: [],
    },
    {
        variant: "Yellow Gold",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 4,
        color: "Yellow Gold",
        size: "One Size",
        quantity: "1",
        likes: 45,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-oro-gold-cufflinks",
        images: [],
    },
    {
        variant: "Yellow Gold",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 3,
        color: "Yellow Gold",
        size: "One Size",
        quantity: "1",
        likes: 19,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-oro-gold-cufflinks",
        images: [],
    },
    {
        variant: "Rose Gold",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "Rose Gold",
        size: "One Size",
        quantity: "2",
        likes: 48,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-oro-gold-cufflinks",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Olive Suede",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 5,
        color: "Olive",
        size: "XL",
        quantity: "1",
        likes: 29,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-oro-suede-bomber-jacket",
        images: [],
    },
    {
        variant: "Navy Suede",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Navy",
        size: "XL",
        quantity: "2",
        likes: 13,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-oro-suede-bomber-jacket",
        images: [],
    },
    {
        variant: "Navy Suede",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 5,
        color: "Navy",
        size: "M",
        quantity: "2",
        likes: 44,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-oro-suede-bomber-jacket",
        images: [],
    },
    {
        variant: "Olive Suede",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 4,
        color: "Olive",
        size: "XL",
        quantity: "2",
        likes: 12,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-oro-suede-bomber-jacket",
        images: [],
    },
    {
        variant: "Midnight Blue",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Midnight Blue",
        size: "L",
        quantity: "1",
        likes: 2,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-lumiere-lace-overlay-dress",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Blush Pink",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 5,
        color: "Blush Pink",
        size: "L",
        quantity: "1",
        likes: 35,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-lumiere-lace-overlay-dress",
        images: [],
    },
    {
        variant: "Blush Pink",
        review: "Exquisite design, looks much more expensive than it actually is.",
        rating: 5,
        color: "Blush Pink",
        size: "S",
        quantity: "1",
        likes: 45,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-lumiere-lace-overlay-dress",
        images: [],
    },
    {
        variant: "Midnight Blue",
        review: "A masterpiece of fashion. It is exactly as described and looks amazing.",
        rating: 4,
        color: "Midnight Blue",
        size: "L",
        quantity: "2",
        likes: 19,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-lumiere-lace-overlay-dress",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Blush Pink",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 5,
        color: "Blush Pink",
        size: "S",
        quantity: "2",
        likes: 10,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-lumiere-lace-overlay-dress",
        images: [],
    },
    {
        variant: "Champagne",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 3,
        color: "Champagne",
        size: "L",
        quantity: "2",
        likes: 34,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-lumiere-satin-midi-skirt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Champagne",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 5,
        color: "Champagne",
        size: "M",
        quantity: "1",
        likes: 13,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-lumiere-satin-midi-skirt",
        images: [],
    },
    {
        variant: "Champagne",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 5,
        color: "Champagne",
        size: "XL",
        quantity: "2",
        likes: 17,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-lumiere-satin-midi-skirt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Champagne",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 3,
        color: "Champagne",
        size: "L",
        quantity: "1",
        likes: 47,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-lumiere-satin-midi-skirt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Champagne",
        review: "Perfect for evening events. Fits true to size and feels very luxurious.",
        rating: 5,
        color: "Champagne",
        size: "XS",
        quantity: "1",
        likes: 10,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-lumiere-satin-midi-skirt",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 5,
        color: "Black",
        size: "2",
        quantity: "2",
        likes: 36,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-lumiere-crepe-tailored-trousers",
        images: [],
    },
    {
        variant: "Black",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 4,
        color: "Black",
        size: "2",
        quantity: "2",
        likes: 47,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-lumiere-crepe-tailored-trousers",
        images: [],
    },
    {
        variant: "Black",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 5,
        color: "Black",
        size: "8",
        quantity: "2",
        likes: 10,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-lumiere-crepe-tailored-trousers",
        images: [],
    },
    {
        variant: "Black",
        review: "Nice item, but slightly tighter than expected. Still beautiful though.",
        rating: 4,
        color: "Black",
        size: "8",
        quantity: "1",
        likes: 18,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-lumiere-crepe-tailored-trousers",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Tortoiseshell",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 4,
        color: "Tortoiseshell",
        size: "One Size",
        quantity: "1",
        likes: 42,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-lumiere-vintage-inspired-sunglasses",
        images: [],
    },
    {
        variant: "Tortoiseshell",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 5,
        color: "Tortoiseshell",
        size: "One Size",
        quantity: "2",
        likes: 1,
        userEmail: "lux-seed-customer-05@example.com",
        productSlug: "lux-lumiere-vintage-inspired-sunglasses",
        images: [],
    },
    {
        variant: "Tortoiseshell",
        review: "Beautiful color and texture. Will definitely purchase from this brand again.",
        rating: 5,
        color: "Tortoiseshell",
        size: "One Size",
        quantity: "1",
        likes: 3,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-lumiere-vintage-inspired-sunglasses",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Tortoiseshell",
        review: "Worth every penny! My new favorite piece in my wardrobe.",
        rating: 3,
        color: "Tortoiseshell",
        size: "One Size",
        quantity: "1",
        likes: 24,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-lumiere-vintage-inspired-sunglasses",
        images: [],
    },
    {
        variant: "Natural Canvas with Tan Leather",
        review: "A must-have for any wardrobe. Classic and timeless.",
        rating: 5,
        color: "Natural",
        size: "One Size",
        quantity: "2",
        likes: 20,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-lumiere-monogrammed-canvas-tote",
        images: [],
    },
    {
        variant: "Natural Canvas with Tan Leather",
        review: "Excellent material, very soft to the touch. Highly recommend to everyone.",
        rating: 5,
        color: "Natural",
        size: "One Size",
        quantity: "1",
        likes: 29,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-lumiere-monogrammed-canvas-tote",
        images: [],
    },
    {
        variant: "Natural Canvas with Tan Leather",
        review: "Simply gorgeous. The fabric flows beautifully.",
        rating: 5,
        color: "Natural",
        size: "One Size",
        quantity: "2",
        likes: 20,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-lumiere-monogrammed-canvas-tote",
        images: [
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Natural Canvas with Tan Leather",
        review: "Incredible craftsmanship. The details are flawless and it feels premium.",
        rating: 5,
        color: "Natural",
        size: "One Size",
        quantity: "2",
        likes: 5,
        userEmail: "lux-seed-customer-03@example.com",
        productSlug: "lux-lumiere-monogrammed-canvas-tote",
        images: [],
    },
    {
        variant: "Cognac",
        review: "Good design, but it took a while to arrive. Still, very satisfied.",
        rating: 4,
        color: "Cognac",
        size: "L",
        quantity: "1",
        likes: 14,
        userEmail: "lux-seed-customer-02@example.com",
        productSlug: "lux-lumiere-lambskin-driving-gloves",
        images: [],
    },
    {
        variant: "Cognac",
        review: "Very elegant and well-crafted. I receive compliments every time I wear it.",
        rating: 4,
        color: "Cognac",
        size: "L",
        quantity: "2",
        likes: 31,
        userEmail: "lux-seed-customer-04@example.com",
        productSlug: "lux-lumiere-lambskin-driving-gloves",
        images: [
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
            {
                url: img,
                alt: "Review image",
            },
        ],
    },
    {
        variant: "Black",
        review: "Absolutely stunning! The quality is incredible and it fits perfectly.",
        rating: 4,
        color: "Black",
        size: "M",
        quantity: "1",
        likes: 16,
        userEmail: "lux-seed-customer-01@example.com",
        productSlug: "lux-lumiere-lambskin-driving-gloves",
        images: [],
    },
];
