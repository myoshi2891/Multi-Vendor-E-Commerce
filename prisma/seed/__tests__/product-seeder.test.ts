...
describe("ALL_SEED_PRODUCTS バリデーション", () => {
    ...
    it("全categoryUrlが存在するカテゴリノードを参照していること", () => {
    ...
    it("全categoryUrlがリーフ（子を持たないノード）を指していること", () => {
        // 子を持つノードへの紐づけは design.md Q5 のリーフ強制に反する。
        // 強制自体は plan 067 以降でアプリ層に入るが、シードは先に条件を満たしておく。
        const parentUrls = new Set(
            SEED_CATEGORIES.map((c) => c.parentUrl).filter(
                (u): u is string => u !== undefined
            )
        );
        for (const p of ALL_SEED_PRODUCTS) {
            expect(parentUrls.has(p.categoryUrl)).toBe(false);
        }
    });
