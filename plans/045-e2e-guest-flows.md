...
2. **compare: 追加 → 比較表示 → クリア** —
   - `/browse` へ goto → `page.getByTestId(`product-card-${seed.product.slug}`)` を `hover()`
     （アクションボタンは hover オーバーレイ内のため必須）
   - `getByRole("button", { name: "Add to compare" })` をカード内で click →
     toast（`Added to compare` 系）または `aria-pressed="true"` を確認
...
5. **offers: シードのオファーが表示され /browse へ誘導する** — `/offers` へ goto →
   `E2E Offer` の見出しが visible → click →
   `await page.waitForURL(/\/browse\?offer=e2e-offer/)`
6. **静的ページのコンテンツスモーク** — `/about`・`/contact`・`/customer-service` の各ページで
   `page.getByRole("main")` 内に見出し（`getByRole("heading")` の first）が visible である
   ことを for ループで確認（404/空白ページの検出が目的。文言は assert しない）。
