...
- **通知の「送信」失敗が主処理（注文更新等）を失敗させないこと** — 送信（外部発火）は主処理の
  ...
  > 両者を混同すると Q4 の選択肢 1（原子的 Outbox = 同一 tx）と選択肢 3（記録できなければ
...
4. **送信の実行モデル**: (a) 主処理後の inline **送信**（`$transaction` の外。ただし
   ...
   **「実行環境の制約」§ と同じ前提で判断すること** —— デプロイ先は未確定なので、
   ...
   >
   > **選択肢 1 と 3 は「遵守すべきリポジトリ規約」§ の「通知の『送信』失敗が主処理を
   > 失敗させないこと」と衝突しない。** 同項は**送信**（外部プロバイダ発火）に限った制約であり、
   > **記録**（行の書き込み）失敗で主処理を巻き込むことは本項が意図して許している。
   > design doc では両者を必ず別の失敗モードとして扱い、障害挙動表でも列を分けること
   > （混同すると「規約に反するから 1 も 3 も採れない」となり、残る 2 だけが
...
- [ ] 「通知の**送信**失敗が主処理を壊さない」ことの設計上の保証（実行モデルの選定根拠）が
      明記されている（**記録**失敗は別。Q4 の選択肢 1/3 は記録失敗で主処理を巻き込むが、
      それは本項の違反ではない）
