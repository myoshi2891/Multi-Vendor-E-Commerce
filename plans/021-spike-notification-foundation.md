...
4. **送信の実行モデル**: (a) 主処理後の inline **送信**（`$transaction` の外。ただし
   通知行の**永続化**は下の必須制約 1〜3 のいずれかで担保すること —— 「行の書き込み失敗は
   ログのみ」は選べない）、(b) Outbox テーブル + cron 再送、のどちらを初期実装にするか。
   **「実行環境の制約」§ と同じ前提で判断すること** —— デプロイ先は未確定なので、
   ...
   > 巻き込まれ、tx 外なら記録漏れが起きうる。選んだ実行モデル（inline / Outbox）で
   > **「永続化失敗時に通知が黙って消えない」不変条件がどう成立するか**を、障害挙動表
   > （プロバイダ停止・タイムアウト・部分失敗に加え **行書き込み失敗**の列）に明記する。
   >
   > **（必須制約）「行の書き込み失敗をログのみで握り潰す」inline 案は選べない。** 主処理 tx の
   > **外**で Notification/Outbox 行の書き込みが失敗した場合、ログだけでは通知を復旧できない
   > （記録が消える）。したがって Q4 の完了条件として、次の**いずれか**を満たす方式のみ採用可とする:
   > 1. **原子的 Outbox**: 通知行の書き込みを主処理と同一 `$transaction` に入れ、主処理と生死を共にする
   >    （その後の実送信は Outbox から cron/worker で再送）。
   > 2. **再試行可能な永続キュー**: 書き込み失敗が失われないよう、再試行・DLQ を持つ永続キューへ投入する。
   > 3. **主処理を失敗させる契約**: 通知行を記録できないなら主処理自体を失敗（ロールバック）させ、
   >    「成立したのに通知記録が無い」状態を作らない。
   >
   > したがって Q4 の (a) は「**inline 送信 + 1〜3 のいずれかによる永続化**」という形でのみ
   > 成立する。**「tx 外 inline 送信・行の書き込み失敗はログのみ」という素の (a) は選択肢では
   > ない**（上記いずれの保証も満たさないため）。Q4 の選択肢を書き写すときは、この条件を
   > 落とさないこと —— 排除済みの案が選択肢一覧に残っていると、実装プランが素の (a) を
   > 「plan で認められた選択肢」として採用しうる。
...
- [ ] **永続化（Notification/Outbox 行の書き込み）失敗時に通知が黙って消えない**保証が、
      選んだ実行モデル（inline / Outbox）の障害挙動表に明記されている（Q4）
