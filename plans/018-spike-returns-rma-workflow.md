...
1. **RMA エンティティの形**: `ReturnRequest`（仮称）が持つべき構造 —
   ...
   >   「同じキーで違う内容を送る」攻撃/バグがすり抜けて重複 RMA ができる（冪等性の目的と逆）。
   >
   ...
   >   - **複合キーの相方である `userId` の NULL 可否も同時に確定すること。** NULL の穴は
   >     制約の**どの列**に空いても同じように開く —— `idempotencyKey` を NOT NULL にしても
   >     `userId` が nullable なら、`userId IS NULL` の申請同士は一意制約で衝突せず、
   >     `@@unique([userId, idempotencyKey])` は**その部分集合に対して無効**になる。
   >     「`idempotencyKey` だけ締めれば冪等」と読めてしまうため、Q1（対象 OrderItem は
   >     複数可か）と同様に**本制約の設計と同時に**決めること:
   >     - ゲスト RMA を認めない（＝ `userId` は常に必須）なら `userId String`（NOT NULL）。
   >       これが既定の想定であり、既存の `Order.userId` とも整合する。
   >     - ゲスト RMA を認めるなら `userId` だけでは subject を同定できないので、
   >       複合キーの主語を別の NOT NULL 列（注文トークン等）へ置き換える。
   >       **nullable な `userId` を複合キーに残したまま「冪等」と称さないこと。**
   ...
   >
   >   これで**同一 subject・同一 target への重複作成**はスキーマが弾き、入力の同一性は
   >   衝突時の比較が保証する（旧版は「入力ハッシュへスコープし」と書きながら制約例に
   >   ハッシュが無く、実装形として成立していなかった）。
   >
   >   **⚠️ 複合一意制約を「認可の保証」として説明しないこと（2026-08-01 訂正）。**
   >   旧版はここを「subject(`userId`)・target(`orderItemId`) は**スキーマが保証し**」と
   >   締めていたが、これは**誤り**であり、spike の実装者に認可チェックの省略を促す。
   >
   >   `@@unique([userId, orderItemId, idempotencyKey])` が保証するのは
   >   **「同じ 3 つ組の行が 2 つできないこと」だけ**である。攻撃者が `userId` に
   >   **他人の id を詰めて**送った場合、その行は自分の行と衝突しないので
   >   **制約を素通りして INSERT が成功する** —— 一意制約は「その `userId` が
   >   リクエスト送信者と同一人物か」を一切見ない。同じことが `orderItemId` にも言え、
   >   他人の OrderItem を指した RMA も制約上は正当な新規行になる。
   >
   >   認可はスキーマではなく **Server Action 側**で閉じること:
   >
   >   - `userId` は**リクエストボディから受け取らない**。`requireUser()`
   >     （`src/lib/auth-guards.ts`）が返す認証済み主体を使う
   >   - `orderItemId` は、その OrderItem が属する Order の所有者が上記主体と
   >     一致することを**クエリの `where` に畳み込んで**検証する
   >     （`docs/testing/SECURITY_GAP_REPORT.md` §5.2 の 3 階層 IDOR パターン）
   >
   >   一意制約の役割は**冪等性（重複防止）**であって認可ではない。両者は直交する
   >   ので、片方があってももう片方は要る。
   >
