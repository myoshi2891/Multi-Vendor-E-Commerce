...
| 新 spec 実行（chromium） | `bash scripts/e2e/run-local.sh tests/e2e/a11y --project=chromium` | 既存 4 + 新 3 passed（認証系 2 spec は plan 042 完了状況に依存） |
...
→ 新 3 spec + seller-apply + sign-in が passed（checkout / profile は plan 042
全体の完了状況に依存 — 042 未完了環境ではその 2 件の fail は本プランの回帰ではない）
...
- [ ] 新 3 spec が chromium で passed（違反ゼロ、disable 追加なし）
