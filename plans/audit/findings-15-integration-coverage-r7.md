...
### [TESTS-24] Clerk webhook `user.deleted` の `db.user.deleteMany` — RESTRICT / CASCADE / SET NULL 混在の FK 連鎖が実 DB 未検証。注文・レビュー・住所・店舗持ちユーザーは削除不能（P2003 → 500 → Svix のリトライ枯渇 + PII 残存）
...
- **Impact**: **注文・レビュー・住所・店舗のいずれか 1 件でも持つユーザーが Clerk 上で
  アカウント削除すると、DB 側の削除は P2003 で永続的に失敗し、webhook は 500 を返し続ける。
  Svix は設定上限までリトライを反復し、最終的に配信失敗として記録される
  （以降は manual retry を打たない限り再送されない）。結果としてユーザーの PII
  （name/email/picture）は DB に残存し続ける**
  （GDPR 等の削除要求と直接衝突するコンプライアンス隣接事案）。
  > **「無限リトライ」ではない**: Svix のリトライは有限（既定のリトライスケジュールに従い、
  > 使い切ると failed として記録され、ダッシュボードからの手動再送か
  > Recover 操作が必要になる）。**リトライが尽きた後は「静かに失敗したまま放置される」**
  > という点がむしろ実務上の問題で、無限に再送され続けるより発覚が遅れる。
  > リトライ回数・スケジュールは Svix 側の設定に依存するため、具体値が必要なら
  > アプリの Svix 設定を確認すること。一方 Cart/Wishlist/
...
- **Fix sketch**: `tests/integration/user-deletion-webhook.test.ts` を新設。route の POST を
  ...
  ShippingAddress 持ちも同様に 500、
  ④**Store 保有ユーザー → 500 + User・Store とも無傷**（`Store.userId` は :640 で RESTRICT。
  Impact 文が RESTRICT 群の筆頭に挙げているのに検証シナリオから漏れていた経路であり、
  **販売者側の PII 残存**を示す。修正方向が顧客側と別設計（店舗の所有権移譲・閉鎖フロー）に
  なりうるため独立ケースとして立てる）、
  ⑤存在しない userId → 200（deleteMany の冪等性）、
  ⑥SupportTicket 持ち（Order なし）→ 200 + ticket.userId が NULL 化、を実 DB で固定する。
  ①では **フォロー中間テーブル（`_UserFollowingStore`）の解消**も assert すること
  （implicit M2M は直接クエリできないため Store 側の `_count.followers === 0` で確認する。
