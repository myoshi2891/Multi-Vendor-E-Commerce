...
- **Evidence**: lcov 実測 — `profile.ts` は **Lines と Branches が別指標**なので分けて読むこと:
  | 指標 | 実測 | 意味 |
  |---|---|---|
  | **Lines** | **59 / 87（67.3%）** | 実行可能行のうち 59 行がカバー済み。未カバーは 28 行 |
  | **Branches** | **18 / 26（69.2%）** | 分岐のうち 18 がカバー済み。未カバーは 8 分岐 |
  > `59/87` は **Lines** の内訳、`18/26` は **Branches** の内訳であり、
  > 一方が他方の詳細ではない（`18/26` = 69.2% が Branches の率に一致する）。
  > 「59/87 → 18/26」のように矢印で連結すると、同じ指標の内訳や因果に読めてしまう。
  > **本 finding の対象は未カバーの 8 分岐（catch 系）**であり、行数ではない。
...
- **`src/components/ui/chart.tsx` Branches 6.5%**: shadcn/ui プリミティブ。snapshot テスト
  ...
  分岐網羅の増分価値は低い。**却下**。
- **`src/components/dashboard/forms/product-details.tsx` 0%（**計測対象** 169 行 / 392 branches）**:
  ...
  > **規模の指標が 2 系統ある点に注意**（[`findings-05-tech-debt.md`](findings-05-tech-debt.md)
  > TECHDEBT-02 の同注記と整合させること）:
  > - **169 行 / 392 branches** = 本行の値。**lcov（istanbul）の計測対象**であり、
  >   実行可能ステートメントとして数えられた行。**テスト到達度**を論じるときはこちら。
  > - **1382 行** = `product-details.tsx:1-1382`（findings-05 TECHDEBT-02）の**物理行数**。
  >   import・JSX・空行を含む。**god component としての分割の必要性**を論じるときはこちら。
  >
  > 両者は別指標であり矛盾ではない（lcov は JSX マークアップや import を計測対象に
  > 数えない）。片方の数字で他方を主張すると議論が噛み合わなくなる。
