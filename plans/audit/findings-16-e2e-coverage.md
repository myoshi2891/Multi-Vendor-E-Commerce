...
- **Effort**: **S–M**（**拡大理由と一致させた見積もり**。修正は `tests/` 変更のため
  本ラウンドでは**プラン化のみ**）
  > **なぜ S ではないか**: 当初の S は「`auth.ts` の locator を 1 箇所堅牢化する」前提の
  > 値だったが、上の再監査（2026-07-11）で**同じ壊れた locator が 4 spec に
  > インライン複製**されていることが判明し、**auth.ts 単独修復では 16 件中 2 件しか
  > 回復しない**ことが分かった。スコープは既に
  > 「共有サインイン関数の抽出 + **5 サイト置換**」へ拡大している（plan 042）。
  > **拡大した作業量に Effort を追従させないと、見積もりと計画が食い違ったまま残る**。
  - 内訳:
    | 作業 | 規模 |
    |---|---|
    | 共有 `signInWithPassword` の抽出 + locator 堅牢化（Clerk カード内スコープ `.cl-signIn-root` + ハイドレーション待ち + `exact: true` + `waitForURL`） | S |
    | 5 サイト（`auth.ts` / `stock-decrement.spec.ts:147` / `messages.spec.ts:60` / `seller-onboarding.spec.ts:79,180` / `platform-coupon.spec.ts:114`）のインライン複製を共有関数呼び出しへ置換 | S–M（呼び出し文脈がそれぞれ異なる） |
    | 3 ブラウザでの回復確認（16 件 + a11y sign-in 1 件） | S（wall-clock は長い） |
