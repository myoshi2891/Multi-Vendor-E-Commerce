...
- **Fix sketch**: `tests/integration/shipping-address-default.test.ts` を新設。①既存住所を
  ...
  作成 → **既存 default が残存し 2 行になる**（現挙動の characterization。修正プラン向けの
  既知ギャップとしてコメントで明記）、③他ユーザーの住所 id を指定した上書きが所有権検証で
...
- **Fix sketch**: `tests/integration/product-browse.test.ts` を新設。①category/subCategory
  フィルタの絞り込み、②size / color のネスト some、③minPrice/maxPrice 境界（minPrice 単独 =
  `Infinity` 経路の characterization — エラーになる場合は STOP して所見報告）、④insensitive
  検索（name/description/variantName）、⑤ページング（totalCount / totalPages / skip）、
  ⑥存在しない category URL でフィルタ脱落 → 全件返却（現挙動の characterization）、を検証。
  → **plan 039**
