...
- **現 HEAD の実装状態（実測で確定）**: **未実装**。`updateOrderItemStatusAsAdmin` 内に
  ...
  > **行番号のドリフトに注意**: 監査時点（2026-07-03 / HEAD `f9752c0`）は
  > `order.ts:538`、2026-07-11 時点では `order.ts:530`、**2026-07-18 実測では
  > `order.ts:509`** と移動を続けている。**このファイルに書かれた行番号は
  > すべて特定時点のスナップショットであり、現在値として読まないこと**
  > （[`VETTED_FINDINGS.md`](VETTED_FINDINGS.md) 冒頭のラウンド継続運用参照）。
  > プラン化時は行番号ではなく **TODO 文字列で grep** して位置を特定すること:
  > `grep -n "TODO(在庫連動・スコープ外)" src/queries/order.ts`
  >
  > 3 回の測定で 3 回とも異なる値になっている事実自体が、行番号を追記し続ける
  > 運用が機能しないことを示している。次回以降は**値を更新せず grep 手順のみを
  > 参照**すること。
