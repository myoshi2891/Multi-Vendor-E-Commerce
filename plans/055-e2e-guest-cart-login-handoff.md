...
5. **サーバー往復の確認 — localStorage を持たない新規コンテキストで検証する**:

   > **`page.reload()` ではサーバー保存を検証できない。** reload は**同一ブラウザ
   > コンテキスト**のままなので localStorage が残り、Zustand ストアがそこから再水和して
   > 同じ商品名を描画する。つまり **`saveUserCart` が完全に壊れていて DB に 1 行も
   > 書かれていなくても、このテストは green になる**。「表示元がサーバー DB の Cart で
   > あること」を主張するには、**クライアント側の永続状態を持たない**新しい
   > コンテキストで開く必要がある。

```typescript
// 同じユーザーでサインイン済みの、localStorage を持たない新規コンテキストを作る
const freshContext = await browser.newContext();
try {
    const freshPage = await freshContext.newPage();
    await auth.signIn(freshPage); // 新コンテキストなので改めてサインインする

    // localStorage は空。ここで商品が見えるなら、供給源はサーバーの Cart しかありえない
    await freshPage.goto("/checkout");
    await expect(freshPage.getByText(seed.product.name)).toBeVisible({ timeout: 15000 });
} finally {
    await freshContext.close();
}
```

   > テスト関数のシグネチャを `async ({ page, browser })` にして `browser` を受け取ること。
   > `freshContext.close()` は `finally` で必ず行う（リークするとワーカーを跨いで
   > 後続テストを不安定にする）。
   > **補足**: 新コンテキストでのサインインが重いと感じても、`storageState` を使い回すと
   > localStorage ごと復元してしまい検証が無意味になるため、ここでは使わないこと
   > （cookie のみを移す手段が無い限り、素直に再サインインするのが安全）。
...
- 新規: `tests/e2e/cart-login-handoff.spec.ts` に 1 テスト（ゲスト構築 → サインイン →
  引き継ぎ → サーバー保存 → **新規コンテキスト（localStorage なし）での再取得**の直列検証）。
...
- [ ] Step 5 が **`browser.newContext()` による新規コンテキスト**（localStorage なし）で
      /checkout を開いている（`page.reload()` だけの検証になっていないこと —
      reload は同一コンテキストで localStorage が残るため、サーバー保存が
      壊れていても green になる）
...
- Step 5 は **localStorage を持たない新規コンテキスト**で検証している点が肝。ここを
  `page.reload()` に「簡略化」すると、クライアント永続からの再水和を見るだけになり、
  サーバー保存が壊れていても green になる（テストの意味が消える）。
- **plan 005（カート整合性の atomic save）が実装されたら**、本テストの Step 5
