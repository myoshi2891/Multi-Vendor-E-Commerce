...
test.describe("sign-out", () => {
    ...
    test("サインアウトするとゲスト状態に戻る", async ({ page }) => {
        ...
        // 認証時のユーザーメニュー（hover で開く）から Sign out
        // トリガーのアクセシブル名は実 DOM で確認する（未認証時の
        // "Sign in / Register" テキストが消えていることが認証成立の裏付け）
        await expect(page.getByText("Sign in / Register")).toBeHidden();
        // user-menu.tsx のトリガー領域を hover → SignOutButton（"Sign out"）を click
        // → ゲスト表示へ復帰
        // （トリガーの特定が難しい場合はヘッダー内の UserButton アバターを hover）
        await page.getByText("Sign out").click({ trial: false });
