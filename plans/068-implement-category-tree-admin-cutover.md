...
4. **深さ上限と循環を実装**（V-7 / V-7b）。`upsertCategory` で `parent.depth + 1 ≤ 4`
   ...
   > **再親子化では旧親と新親の *両方* をロックする。** 新親（子の作成先）だけを
   > 掴むと、`childCount` を減算する**旧親**が無防備なまま残る —— 「A の子 X を B へ
   > 移す」と「A の下に新しい子を作る」が同時に走ると、両者が A の `childCount` を
   > read-modify-write して**片方の更新が失われる**。結果は Step 3 のリーフ強制が
   > 参照する値の破損（「子がいないのに `childCount > 0`」／その逆）であり、
   > 正当なリーフへの紐づけが拒否される・非リーフへの紐づけが通る、という形で出る。
   > **ロック順は id の昇順に固定すること。** 2 件の再親子化が旧親／新親を互い違いに
   > 掴むと循環待ちでデッドロックする。順序を id で正準化すれば、どの経路も同じ順に
   > 待つので循環が生じない（新親側の「子作成ロック」はこの順序の中で取る）。加えて **循環を作る再親子化を拒否**する:
...
8. **テスト**:
   ...
   - **V-5d（並行リーフ化）**: 「商品を L に紐づける」と「L の子を作る」を
     **同時にディスパッチ**しても、非リーフ紐づけが成立しないこと（統合テスト・
     ...
     の並行遷移シナリオと同じ形）。
     加えて **旧親側の競合**も同じ形で固定する: 「A の子 X を B へ再親子化」と
     「A の下に新しい子を作る」を同時にディスパッチし、**A と B の `childCount` が
     どちらも実際の子の数と一致する**こと（旧親をロックしないと A 側で更新が失われ、
     この検算だけが赤になる）。
...
- [ ] `bun run erd:generate` の stderr に orphan WARNING が 0 件、`.drawio` が
      スキーマ変更と同一コミット
- [ ] V-5 / V-5b / V-5c（リーフ強制・create と update の両方、同一 root 内のリーフ差し替えを含む）/
      **V-5d（並行リーフ化 —— 商品紐づけと子ノード作成の同時実行、および再親子化時に
      旧親の `childCount` 不変条件が保たれること）** / V-7（深さ上限）/
