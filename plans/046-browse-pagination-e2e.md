...
- 既存 search-filter の他テスト（`:19` 検索 / `:37` カテゴリフィルタ / `:54` 0 件表示）は
  実データ方式 — 追加 seed が件数 assert を壊さないか Step 4 で回帰確認する。
...
test("ページネーションで次ページに遷移できる", async ({ page }) => {
    ...
    await page.goto(`/browse?category=${seed.paginationCategory.url}`);
    // 1 ページ目: 10 件
    ...
    await page.goto(`/browse?category=${seed.paginationCategory.url}&page=abc`);
    await expect(page.locator('[data-testid^="product-card-"]')).toHaveCount(10);
});
```
...
**Verify**:
- `bash scripts/e2e/run-local.sh tests/e2e/search-filter.spec.ts` → 15 passed
...
- E2E: 書き換えた 1 テスト（1 ページ目 10 件 → Next → 2 ページ目 2 件 → 不正値正規化）。
- 既存 search-filter 4 テスト + purchase-flow / layout-chrome の回帰 green。
...
- [ ] search-filter spec が skip 0・3 ブラウザ 15 passed
