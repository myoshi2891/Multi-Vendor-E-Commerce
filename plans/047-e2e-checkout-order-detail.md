...
> 使う（計算ロジックの一元管理）。
>
> **経路 B のセント整数は、規約に明記された唯一の例外である（2026-08-02 昇格）。**
> 根拠と適用範囲の SSOT は [`.claude/steering/tech.md`](../.claude/steering/tech.md) の
> **「金額規約の唯一の例外: E2E の表示文字列検算（セント整数）」** 節。以下は要約であり、
> 齟齬がある場合は tech.md を正とする。
>
> 金額規約（`Decimal(12,2)` 必須 / 演算は `Prisma.Decimal` メソッド / 中間集計での
> `.toNumber()` 禁止）が防ごうとしているのは「DB の正確な 10 進値を `number` に落として
...
> **失われた精度は `Decimal` を被せても戻らない**ため、ここで `Prisma.Decimal` を使っても
> 厳密性は 1 ビットも増えず、依存だけが増える。小数第 2 位までと決まった文字列は、
> **パース時に 1 度だけ丸めてセント整数化**すれば以降の加減算が厳密（整数演算）になる。
>
> **以前この段落は「規約対象外」と自称していたが、それでは規約は曲がらない（2026-08-02 修正）。**
> 金額規約は `**/*.{ts,tsx}` 全体に掛かるので、例外は規約側に持つ必要があった。
> あわせて「桁区切りカンマは入らず常に `$X.XX` 形式」という記述も削除した ——
> 下の実装の正規表現はカンマ形式を受理しており、**同じ段落の中で前提と実装が食い違って**いた。
>
> **境界の要約**: `Decimal` 列の値を計算する → 規約適用（経路 A）。
> 丸め済みの表示文字列どうしの関係を検算する → 規約の明示例外・セント整数（経路 B）。
...
const parseMoneyToCents = (text: string): number => {
    ...
    //
    // **整数部は「カンマ無し」と「1〜3 桁始まりのカンマ形式」を分けて表現する。**
    // `[0-9]+(?:,[0-9]{3})*` だと先頭の桁数が無制限になり、`$1234,567.89` のような
    // **不正な桁区切り**を受理して `1234567.89` を返してしまう。本実装は
    // 「厳密な形式と fail-fast」を要求しているので、異常な表示は例外にすべきである。
    //
    // **正規表現だけでは fail-fast にならない。** `match` は最初に一致した位置を
    // 返すので、不正なトークンには「throw」ではなく「読み飛ばし」で反応する。
    // `"$1,23.45 $12.34"` は 1 つ目を素通りして `1234` を返す —— 上の各行が
    // 「throw する」と謳っている入力が、後ろに正常値を 1 つ置くだけで復活する。
    // 検算対象は 1 つの表示値なので、`$` の個数を 1 に固定して経路ごと塞ぐ。
    //
    // 実測（2026-08-02・node）— 正常系は旧形と完全一致し、不正入力だけが変わる:
    //   入力                        旧            新
    //   $12.34                      1234          1234
    ...
    //   $1234,567.89                123456789     throw  ← 桁区切り修正で閉じた穴
    //   $1,23.45 / $12.345 / $12.34.56 / $12.34abc   throw  throw
    //
    // 個数固定ぶんの実測（2026-08-02・node。旧＝桁区切り修正済み・個数固定なし）:
    //   $1,23.45 $12.34             1234          throw  ← 不正トークンの読み飛ばし
    //   $12.34 $56.78               1234          throw  ← どちらの値か決まらない入力
