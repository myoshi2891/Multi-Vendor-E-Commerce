...
> 原本: [../../audit/findings-02-security.md](../../audit/findings-02-security.md)
...
- **Evidence**: `next.config.mjs` — `images.remotePatterns` のみ設定、`headers()` ブロックなし（CSP / X-Frame-Options / frame-ancestors / X-Content-Type-Options / Referrer-Policy / HSTS なし）。アプリはサニタイズ済みだが `dangerouslySetInnerHTML` で販売者 HTML を描画（`src/components/store/product-page/product-description.tsx:22`）、checkout/profile も扱う。
...
- **Fix sketch**: `next.config.mjs` の `headers()` に `Content-Security-Policy-Report-Only` を先行導入 + `X-Content-Type-Options: nosniff` / `Referrer-Policy` / frame-ancestors。検証後に enforcing へ昇格。
