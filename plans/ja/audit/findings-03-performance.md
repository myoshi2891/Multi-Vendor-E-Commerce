...
> 原本: [../../audit/findings-03-performance.md](../../audit/findings-03-performance.md)
...
- **Fix sketch**: `retrieveProductDetails` 後に独立呼び出しを `Promise.all`、view increment は await せず `void`。
...
- **Fix sketch**: `getDominantColors` を専用モジュール（`src/lib/color.ts`）へ抽出、or 関数内で `const ColorThief = (await import("colorthief")).default`。
