...
- 現行コードが既にアイテムレベルの遷移で在庫復元しており、**かつ下のクローズ条件 (1)(2)(3) が
  すべて成り立つ**（increment が実在し／ちょうど一度だけのガードがあり／その順序を証明する
  テストがある）。下の但し書きを参照 — TODO が埋まっただけでは本 spike をクローズする根拠にならない。
  **「ちょうど一度だけに見える」だけでは停止しないこと**: (3) のテストを欠いたまま停止すると、
  ガードが未検証のまま spike をクローズすることになる（それは (1) だけを満たす状態と区別できない）。
...
> **TODO が埋まっただけでは本 spike はクローズできない。** 1 つ目の STOP 条件が問うているのは
> **挙動**であり、コメントの有無ではない。TODO を消して `orderItem.update` の隣に
> `restockOrderItems` 呼び出しを置けば「TODO が埋められている」は満たされるが、設計上の問いは
> 何ひとつ解決していない。しかもその素朴な実装こそが、アイテムをアイテムレベルでキャンセルした後に
> 注文を注文レベルで返金したときに**二重復元する**版である。このシナリオは本プラン自身が
> 「鍵となる受け入れゲート」と位置づけているもの（Maintenance notes 参照）。
>
> クローズしてよいのは以下が**すべて**成り立つ場合のみ:
>
> 1. 在庫復元をトリガーする `ProductStatus` へのアイテムレベル遷移で、実際に `Size.quantity` が
>    increment される（コメントではなくコードパスを読んで確認すること）。
> 2. その increment が、アイテムレベル経路と注文レベル経路をまたいで**ちょうど一度だけ**起きるよう
>    ガードされている — つまり無条件呼び出しではなく状態遷移ガードが存在する。
> 3. 「アイテムキャンセル → 注文返金」の順序について (2) を証明するテストがある。そのテストが
>    無い限りガードは未検証であり、spike にはまだやることが残っている。
>
> (1) は成立するが (2) または (3) が成立しない場合は、**クローズしないこと** — 既存実装に対する
> ちょうど一度だけのガードの設計と証明へと spike を再スコープし、その再スコープを報告する。

**ドリフト注記（2026-07-19）**: 上で `order.ts:538` として引用されている TODO は現在
[`order.ts:509`](../../src/queries/order.ts) にあり、`updateOrderItemStatusAsAdmin` の内部で
**未だ埋められていない** — したがって本 spike は有効なまま。一方で**注文レベル**の在庫復元は
