...
4. **sizes 全置換の下流副作用: Wishlist は SetNull・CartItem は stale**:
   ...
   弾かれる経路の前提を固定）
5. **tx 原子性: 途中失敗で子テーブルが置換前のまま**:
   `buildUpdateInput(..., { categoryId: "nonexistent-category-id" })` → reject
   （tx 内の `category: { connect: ... }` が P2025 で失敗）。
   旧 spec（"old-spec"）/ 旧 question / 旧 size が**すべて残存**し、新 spec が 1 件も
   ないことを assert（半置換が起きない = ロールバックの実証）
...
- [ ] シナリオ 5 に「reject + 旧 spec/question/size 残存 + 新行ゼロ」の assert が存在する
