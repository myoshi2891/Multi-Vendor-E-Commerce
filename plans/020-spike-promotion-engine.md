...
> スキーマ要件を固定しない:
> - **EU（Omnibus 指令）**: 値引き表示時に「直近 30 日間の最低価格」を参照価格として提示する義務 →
>   価格履歴は少なくとも 30 日ぶんの価格変更を保持し「期間内最低価格」を算出できる必要がある。
> - **US（FTC）/ 日本（景品表示法・二重価格表示）** 等は基準が異なる（「実売実績のある通常価格」等）。
> - **現状の本プロジェクトの対象法域は未宣言**（`product.md` に記載なし）。
>   → spike は法域を**「未定・実装前に要確認」プレースホルダ**として明記し、
>   「もし EU 基準なら 30 日最低価格を出せる粒度が必要」等の**条件付き要件**として設計する。
