...
5. **セール自動終了の実行方式**: `saleEndDate` / キャンペーン期限の失効を
   ...
   （常駐プロセス不可）を踏まえて確定する。
   > **表示方式に関わらず、価格の権威的確定（checkout の金額オーソライズ）時点で失効を
   > 再検証すること**。(a) の読み取り時判定は表示用、(b) の cron フラグは遅延しうる —
   > いずれもオーソライズ時の真値ではない。注文成立サーバーアクションで `saleEndDate` /
   > 期限を再評価し、失効済みセール価格を確定させない（display と authorization を分離）。
   > **さらに期限の再検証だけでは不十分で、割引価格そのものをサーバー側で再導出すること** ——
   > 割引率・参考価格は表示時とオーソライズ時で正当に変わりうるため、クライアント/表示時の
   > 価格を信頼せず、注文成立時に権威的ソースから実際の金額を再計算して確定する。
   > これは本リポジトリ既存の総額サーバー再導出パターン（`placeOrder` / クーポン適用）と同型。
   >
   > **権威的ソースは 2 系統あり、どちらか一方では足りない**:
   > - **請求額**（実際に課金する金額）: Size/variant の現在価格 + 適用中のセール規則。
   > - **参考価格**（取り消し線で見せる「元値」）: **Q4 で定める価格履歴由来**
   >   （直近 N 日の最頻値等）。**現在の価格表から導出してはならない** —— 二重価格表示の
   >   規制対応が要求するのは「その価格で実際に売っていた実績」であり、現在の価格表には
   >   その情報が無い。現在価格から逆算した参考価格は、値上げ直後に架空の割引率を表示する
   >   経路になる。
   >
   > したがって Q4 を「記録のみ（表示は後続）」に絞る判断を採る場合、**参考価格の表示も
   > 同じ後続スコープに入る**（履歴が無い状態で取り消し線価格を出す実装を先行させないこと）。
...
- [ ] **checkout オーソライズ時点の再検証・再計算契約**が design doc に節として存在し、
      次の 3 点すべてを満たす（Open question 5 の (必須) blockquote を完了ゲートへ昇格）:
  - [ ] **期限の再評価**: 注文成立サーバーアクションで `saleEndDate` / キャンペーン期限を
        再評価する旨が明記されている。読み取り時判定 (a) は表示用・cron フラグ (b) は
        遅延しうるため、**どちらを Q5 で選んでも**オーソライズ時の再評価は免除されない
  - [ ] **請求額のサーバー側再導出**: クライアント/表示時の価格を信頼せず、権威的ソース
        （Size/variant の現在価格 + 適用中のセール規則）から実際の課金額を再計算して
        確定する旨が明記されている（既存の `placeOrder` / クーポン適用と同型であることを含む）
  - [ ] **参考価格の出所**: 取り消し線価格が **Q4 の価格履歴由来**であり、**現在の価格表から
        逆算しない**旨が明記されている。Q4 を「記録のみ」に絞る場合は、参考価格の表示も
        同じ後続スコープに入る（履歴なしで取り消し線価格を出す実装を先行させない）ことが
        design doc とスコープ表の両方に書かれている
