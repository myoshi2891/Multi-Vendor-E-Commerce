...
- **ストアフォロー**（`src/components/store/cards/store-card.tsx`）:
  ...
  - フォロー一覧: `/profile/following`（`src/components/store/profile/following/container.tsx`
    が同じ StoreCard を描画）
...
2. **ストアフォロー: follow → 一覧反映 → unfollow**
   ...
   ※ following 一覧の URL 形式（`/profile/following` か `/profile/following/1` か）は
   `src/app/(store)/profile/following/` のディレクトリ構成を見て確定する。
