   ...
   **属性値の保存契約（スキーマ生成だけでは往復が閉じない）**。動的スキーマを足しただけでは
   ...
   - 保存 `$transaction` 内の同期は**キーをスコープごとに変える**。単値属性は
     ...
     **多値属性は Step 2 の決定に従う** —— upsert ではなく
     ...
     部分更新でゴースト値が残らないこと。
   - **同期削除の対象は「いま編集できる定義」＝ `archivedAt` が未設定の定義に限る。**
     フォームが受け取る定義集合は継承クエリの `archivedAt: null` で絞られているため、
     「送信されなかった定義を消す」を所有先だけで撃つと、**アーカイブ済み定義の値まで
     巻き添えで消える** —— design.md Q7「定義の削除」が「論理削除では値を保持する
     （履歴・再有効化・エクスポートのため）」と定めているのに、商品を 1 回保存しただけで
     その履歴が消えることになる。したがって `$transaction` 内の
     `ProductAttributeValue` / `VariantAttributeValue` の `deleteMany` は、
     所有先（`productId` / `variantId`）に加えて **`definitionId: { in: アクティブ定義の id }`**
     で必ず絞ること。
     - 利用者が明示的に空へした（clear した）値は従来どおり削除する。**「送信されなかった」と
       「空で送信された」を payload の形で区別できるようにする**（前者は同期対象外、
       後者は削除対象）—— 区別できない payload だと上の限定が意味を失う。
     - 回帰テストを追加する: **定義をアーカイブしたあとに商品を保存しても、
       その定義の値が残っている**こと（PRODUCT / VARIANT の両スコープ）。
   - **`$transaction` に入る前にサーバー側で再検証する（型は認可ではない）**。
     ...
     > `archivedAt` 付与が割り込むと、**検証済みのつもりの id が書き込み時点では無効**になる。
