...
2. **新規住所を default 付きで作成すると既存 default が残存する（現挙動の characterization）**:
   `upsertShippingAddress({ ...addressA, id: randomUUID(), default: true })` →
   新行が作成され、`count({ where: { userId, default: true } })` === **2**。
   テストコメントに「本来は 1 であるべき — 修正プラン実行時にこの期待値を 1 に反転する」
   と findings-14 TESTS-21 への参照を書く
...
- [ ] シナリオ 2 に `default: true` 件数 === 2 の assert と TESTS-21 参照コメントが存在する
