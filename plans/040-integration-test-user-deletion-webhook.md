...
1. **Cart / Wishlist / フォローのみのユーザーは削除され、子行が連鎖消滅する（CASCADE）**:
   ...
   `db.cartItem.count` === 0、`db.wishlist.count` === 0。**セラー側は無傷**:
   store / product / variant / size が残存（count 1 ずつ）
...
5. **SupportTicket（orderId なし）持ちユーザーは削除され、ticket が匿名化される（SET NULL）**:
...
6. **存在しない userId は 200（deleteMany の冪等性）**: seed なしで
   `postUserDeleted("user_does_not_exist")` → **status 200**（deleteMany は count:0 で正常終了）

**Verify**: `bun run test:integration -- tests/integration/user-deletion-webhook.test.ts` → all pass（6 テスト以上）
...
- [ ] シナリオ 5 に「200 + ticket 残存 + userId NULL 化」の assert が存在する
...
- **シナリオ 2〜4 のいずれかで削除が成功（200）してしまう** — FK が RESTRICT でなくなっている
  （schema/migration が変わった）。characterization の前提が崩れているので、実際の FK 定義を
  添えて報告
- シナリオ 1 でいずれかの子テーブル行が残存する、またはセラー側リソースまで消える —
  CASCADE 定義のドリフト。実測の残存/消滅テーブル名を添えて報告
...
- 本テストは「注文・レビュー・住所・店舗持ちユーザーは Clerk 削除に DB が追従できない」
  ...
  （PII 匿名化 + 行温存 / ソフト削除 / onDelete 変更のいずれか）、シナリオ 2〜4 の期待値を
...
- 将来 `user.deleted` 経路に匿名化が入ると、シナリオ 5（SupportTicket SET NULL）は
