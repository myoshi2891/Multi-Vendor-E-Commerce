...
>
> **`ORDER BY ts_rank` だけでページングしないこと（安定した tie-breaker が必須）**。
> `ts_rank` は全順序ではない — 同一スコアの行が多数生じるのが普通で（特に短い
> クエリや語が 1 回しか出現しない商品群では**大半が同点**になる）、同点行同士の
> 返却順序を PostgreSQL は保証しない。プラン・並列度・データ配置で実行ごとに
> 変わりうる。
>
> これは体裁の問題ではなく**ページングの正しさの問題**である。`LIMIT/OFFSET` で
> 1 ページ目と 2 ページ目を別クエリとして発行する以上、その 2 回の間で同点行の
> 並びが変われば、**ある商品が両ページに現れ、別の商品がどちらにも現れない**。
> ユーザーには「商品が消えた」「同じ商品が 2 回出る」として観測され、再現も難しい。
>
> spike は次を設計に含めること:
>
...
> 持たないため問題が露見していないだけであり、ファセット + ページングを導入する
> 本 spike で初めて顕在化する。**「今は起きていない」を根拠に省略しないこと。**
...
- `EXPLAIN` を取り、GIN インデックスが使われているかを併せて記録する。
  **ただし Seq Scan を一律に失敗扱いにしないこと。** 開発 DB の行数（seed 直後は数百行規模）では
  プランナが Seq Scan を選ぶのが**正しい**判断であり、インデックスの不在を意味しない。
  インデックスが使える形かどうかを見たいなら、`SET enable_seqscan = off;` を掛けた上で
  GIN が選ばれることを確認するか、本番相当の行数を投入してから再測定する。どちらを採ったかを
  design doc に書き、`EXPLAIN` の結果は「この行数でのプラン」という条件付きで記録する。
