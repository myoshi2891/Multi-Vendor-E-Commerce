...
2. **国を変更するとヘッダー表示が更新される** —
   ...
   - **`waitForResponse` の Promise を option の click より「前」に作り、click 後に await する**:
...
const countryResponse = page.waitForResponse(
    (r) => r.url().includes("/api/setUserCountryInCookies") && r.status() === 200
);

await page.getByRole("option").filter({ hasText: "Japan" }).first().click();
