...
3. **空メールでは送信されない（HTML5 required）** — 別テスト。入力欄を空のまま
   `Sign up` を click → `/api/newsletter` への POST が**発生しない**ことを確認する。

   > **固定の `waitForTimeout` を使わないこと。** 「1s 待って POST が無ければ OK」は
   > 決定論的でない: 遅い環境では 1s 後に POST が飛んで**見逃す（偽陰性）**し、
   > 速い環境では毎回 1s を無駄に払う。待ち時間を伸ばせば安全になるわけでもなく、
   > 「どれだけ待てば十分か」に原理的な答えが無い（これは「起きないこと」を
   > 時間で証明しようとする典型的な失敗）。
   >
   > 代わりに **`page.on("request")` で全リクエストを収集**し、
   > 「送信が起きるなら、それより前に必ず観測できる別の事象」を待ち切ってから
   > 判定する。ここでは **HTML5 バリデーションがブロックした証拠**（フォームが
   > 送信されず、入力欄が invalid のままフォーカスされる）を決定的な基準にする。

```typescript
// click より前から収集を開始する（開始が遅れると取りこぼす）
const newsletterRequests: string[] = [];
page.on("request", (request) => {
    if (request.url().includes("/api/newsletter") && request.method() === "POST") {
...
- [ ] spec に固定待機が無い
      （`grep -n "waitForTimeout" tests/e2e/newsletter.spec.ts` → 0 件。
      「POST が起きない」ことの検証は `page.on("request")` の収集 +
      `expect.poll` による決定的条件で行うこと）
