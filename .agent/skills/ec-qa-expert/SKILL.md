...
| `docs/testing/QA_HANDOFF.md` | Open Issues の状態更新・次着手を明記 | セッション終了時（必須） |
| `docs/testing/COVERAGE_REPORT.md §3` | タスク（A/B/C）の完了記録・ヒートマップ更新 | セル状態（`◯`/`◐`/`✦`）が変化したとき |
| `docs/testing/SECURITY_GAP_REPORT.md` | IDOR/RBAC の発見・修正・テスト追加を記録 | セキュリティ脆弱性の修正時 |
| `docs/PROGRESS.md` | テスト統計（テスト数・型エラー数）を同期 | 統計値が変化したとき |
| `docs/coverage-dashboard.html` | `bun run coverage:dashboard` を実行して再生成 | テストの追加/変更・カバレッジが更新されたとき |

> **重複防止**: 即時 TODO は `QA_HANDOFF.md` のみに書く。`COVERAGE_REPORT.md §3` には「なぜやるか」の根拠のみ。
> 詳細なルールは [`.claude/steering/documentation-guide.md`](../../../.claude/steering/documentation-guide.md) の `docs/testing/ 各ファイルの更新ルール` を参照。
