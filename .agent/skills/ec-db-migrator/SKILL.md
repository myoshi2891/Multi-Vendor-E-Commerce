...
- ✅ **金額フィールドの定義**: 金額（価格、配送料、合計額）を扱うフィールドには必ず `Decimal(12,2)` 型を使用し、`Float` は使用しないこと。
- ✅ **複合ユニーク制約**: 重複登録を防ぐ必要がある場合（例: `ShippingRate` の `storeId`+`countryId`、`Review` の `userId`+`productId`）、`@@unique` インデックスを適切に設定すること。
