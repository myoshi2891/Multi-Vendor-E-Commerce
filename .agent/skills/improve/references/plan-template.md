...
File naming: `<plan-dir>/NNN-short-slug.md`, numbered in recommended execution order. `<plan-dir>` is the directory selected during recon: normally `plans/`, or `advisor-plans/` when `plans/` already serves an unrelated purpose. Use that same directory throughout the plan and index; do not fall back to `plans/` in template text after selecting `advisor-plans/`.
...
- **Effort**: S | M | L
- **Risk**: LOW | MED | HIGH
- **Depends on**: `<plan-dir>/NNN-*.md` (or "none")
