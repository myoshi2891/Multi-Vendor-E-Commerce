...
- Dependency posture: run the ecosystem's audit command (`bun audit`, `npm audit`, `pip-audit`, `cargo audit`) in read-only mode — pick the one matching the project's package manager (this repo is Bun-based, so `bun audit`). Report only critical/high advisories that affect reachable runtime code or build/distribution paths; avoid low-signal audit noise.
