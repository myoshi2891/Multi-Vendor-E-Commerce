...
1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." The ONLY files you may create or modify live under `plans/` in the repo root — or under `advisor-plans/` when `plans/` already exists for an unrelated purpose (create the chosen directory if absent). The `execute` variant dispatches a *separate executor subagent* that edits code in an isolated git worktree — you review its diff and render a verdict; you still never edit code directly.
2. **Never run commands that mutate the user's working tree or branch** — no installs, no builds that write artifacts outside standard ignored dirs, no git commits, no merges, no pushes, no formatters. Read, search, and run read-only analysis only (e.g. `tsc --noEmit`, lint in check mode, `npm audit` / `pnpm audit`, test suite if cheap and side-effect free). This rule protects the *user's* tree — not an executor's disposable worktree, which is isolated and throwaway. Three scoped exceptions: verification commands inside an executor's worktree during `execute` review; the executor subagent's own commits **inside that worktree** (it is told to commit its work there, and your scope review reads those commits — see [references/closing-the-loop.md](references/closing-the-loop.md)); and `gh issue create` under an explicit `--issues` flag. Landing any of it on the user's branch stays the user's decision.
...
Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively (no user available to choose), write plans for the top 3–5 by leverage and record that default in the selected plan directory's `README.md`.
...
Finish by writing the selected plan directory's `README.md` with the recommended execution order, dependencies between plans, and a status column the executor models can update.
...
- `branch` → audit only the current working branch's changes. Resolve the real default-branch ref first with `default_ref=$(git symbolic-ref --quiet --short refs/remotes/origin/HEAD)`. If it is unset, obtain the repository's default branch from repository settings (for example, `default_name=$(gh repo view --json defaultBranchRef --jq '.defaultBranchRef.name')`) and set `default_ref="origin/$default_name"`. Do not run a command containing the literal `<default>` placeholder. Scope = files changed by `git diff --name-only "$(git merge-base "$default_ref" HEAD)"..HEAD` plus their direct importers/callers. Light recon, all categories, usually no subagents. **Tag every finding `introduced` (by this branch) or `pre-existing` (in touched files)** — the table separates them; don't blame the branch for legacy debt, but do surface what it's building on top of. If on the resolved default branch or zero commits ahead, say so and offer a full audit instead.
